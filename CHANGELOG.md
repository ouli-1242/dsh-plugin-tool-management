...
- **注入层也分家：`scene-memory-manager-catalog` → `scene-manager-catalog` + `memory-manager-catalog`**（用户 2026-09-23 裁定）。原来那**一个段是语义单元** —— 它的引导语跨"场景说明"和"记忆条目"做分级授权（场景说明是**约定**、一律照办；条目是**记录**、可能过期），靠的正是"两类内容渲染时分处不同位置"。拆开后用户能**单独关掉记忆段**（省字节）而保留场景说明 —— 原来关掉 `memory` 域是两者一起消失。**场景说明只在场景段出现**（记忆段的场景块只留标题），两段各自完整自洽。三条口径都是用户当场裁定的：
  - **场景段不带授权语**（同日第二次裁定，用户看到**实际注入**后提的）：上一版把它写成"「场景说明」是用户为本机写的约定，一律照办，覆盖你的默认做法"，而这一格的实例是「写代码」「前端相关」这种**标签** —— 让模型"照办一个标签"正是它读不懂那一段的原因。"场景是什么"改由框架线索给（`场景是用户给这台机器配的工作模式；下面是当前启用的那个。`），正文只给一行 `**「web」—— 前端相关**`（`sceneLine`）：场景名从**三次**减到一次（旧版：线索里的"现在处在哪个场景"、引导语里的「web」、标题里的 `## 场景：web`），整段 136 → 81 字符（356 → 211 字节）。**没填说明的场景不带后缀** —— 旧版会补 `defaultSceneDescription` 的默认句，而"用户配置的上下文，当前启用"贴在名字后面读起来像一句真的说明。
...
- **记忆段的授权语压缩 38%**（用户 2026-09-23 第三次裁定：「这段话很多没用信息，浪费 token 和上下文」）：现在是 `**本机记录，可能已过期：冲突时以实际情况和用户当场说的为准；无关不必提。以下就是全部信息。**`。77 → 48 字符（219 → 136 字节），而它**每个场景块各来一次**，所以省的是 N 倍（两个启用场景：每轮省 166 字节 ≈ 42 tok）。**五个功能一个没动** —— ① 这是什么、② 可能已过期、③ 冲突时以实际情况为准、④ 冲突时以用户当场说的为准、⑤ 无关不必提 + 完整性声明（那半句是防探测的唯一手段 —— `constants.ts` 里记着"真正防探测的就是它，必须留"）。删掉的只有修饰与重复：「以下是用户为本机写的记录」→「本机记录」（板块标题与线索已经说过"这是本机的记忆"），两句"冲突时以…为准"合成一句。
- **注入的元指令精简，并补上场景的因果**（用户 2026-09-23 贴出完整注入全文：「感觉还是不对劲，但说不上来」）。量化之后"不对劲"有两个来源：四个板块合计 716 tok/轮，**内容只占 46%**；而**越短的板块包装占比越高**（场景 87%、MCP 76% —— 三行内容配了 98 tok 元指令）。逐项：
  - **场景段补上因果句**：此前它是四个板块里唯一**没有动作句**的 —— 首句只说"场景是什么"，模型读完不知道该拿它做什么。根因是**场景的语义被窄化了**：它其实是"切换六处开关的运行时模式"（`scene-mode-set` 三步：服务器级/来源级 → 工具级/技能级 → 人设 + 备注），而注入里只呈现了它作为"记忆分组标签"的那一面 —— 四个板块并列，看不出场景是"因"、其余四段是"果"。现在 cue 是动作句（与另三段同形），并补一句因果把四段串起来。代价 **+36 B（≈9 tok/轮）** —— 这一处是**变贵**的，买的是语义清楚。（第一版写 +33 B / 8 tok，是用错索引量出来的：复现脚本把 `readIndexSync` 的入参传成了不存在的 `state/` 子目录 → 拿到空索引 → 场景说明丢失。已按正确索引重测更正。）
  - ⚠️ **因果句是**按状态分叉**的，不能写死一句**：场景**启用**（`active`，`rules-set-active`）与场景**进入**（`mode.scene`，`scene-mode-set`）是**两份状态、两个 op**，可以只启用而不进入。记忆按**启用**筛，MCP / 技能 / 人设按**进入**后的档案切 —— 只启用没进入时那三段还是全局配置，说"都已按它筛过"就是假话。所以挪进 `renderSceneCatalog` 的正文（读 `index.mode.scene` 分叉），两种状态各说各自的事实。顺带修掉第二处毛病：它原本写在框架里、排在场景名**之前**，「它」根本没有指代。
  - **记忆段授权语提到板块层，整段只说一次**（不是每个场景块各一遍）：2 个场景就是 272 B ≈68 tok/轮，占记忆段 22%；而且它夹在 `## 场景：X` 与条目之间，把分组和内容隔开了。它管的本来就是整段（「以下就是全部信息」说的是全段）。早前那条「放最顶层会飘在场景之外」的约束**已不成立** —— 那时板块标题还是 `##`、与场景分组平级，现在板块是 `#` 一级，放在两者之间就是明确的板块级说明。省 136 B ≈34 tok/轮（场景越多省越多）。
  - **附件行去掉纯修饰**：`附件目录：…（未注入正文，共 1 个：xxx）` → `附件：…（xxx）`。目录 + 文件名已经把"内容不在这里、要读照路径去读"说清楚了，那半句解释与计数词每轮白付。总数只在**列不全**时才保留。**路径仍必须绝对** —— 模型拿到它要去 `Read`，相对路径等于没给。两行 289 → 201 B，省 22 tok/轮。
  - **MCP 段不再列工具数**：模型不能凭"这台有 N 个工具"做任何事（能调哪些在它自己的 schema 里，按 `mcp__<server>__` 前缀就能对上号），17 B/台 × 每轮是白付的。②档原来写「上次连上时 N 个工具」，那个数字是**历史事实**、措辞上必须与现状区分开；数字没了，这个歧义也不存在了。分档判据**没变**，仍然只认 `liveEnabledToolCount`。省 13 tok/轮。
  - **supersede 统一成一句**：五个域此前各写一份（`本份场景取代…同类场景` / `本份记忆…` / `本份状态…` / `本份目录…`），说的是同一条规则却用了四种措辞。统一为 `本份取代本次会话中更早注入的同类内容。` **收益只有约 6 tok/轮**（21 → 19 字）—— 主要收益是**消除四种措辞**，不是省字节。没并进 cue（那能省一整行 ≈16 tok/段）是因为用户 2026-09-18 定过"权威声明单独成句"：它和动作句是两种东西，合并后容易被一眼带过。
  - 四段合计 **715 → 约 655 tok/轮（−60 tok，−8.4%）**。逐项实测（真实 `~/.dsh` 数据 + 正确索引）：场景 251 → 287 B、记忆 1263 → 1045 B、MCP 513 → 458 B、子智能体 834 → 830 B。
  - ⚠️ **顺带发现：一直用的那个"token 数"口径对中文不准，跨语言比大小会得出错的结论**（用真 BPE 词表实测）。口径是宿主的 `字符数 / 4`（工具表）与我们同形的 `字节数 / 4`（注入）：**英文高估约 10%**（18 个工具 3,439 → 真 ≈3,137 tok），**中文低估约 13%**（注入四段 716 → 真 823；换 cl100k 词表低估 28%）。所以"工具表 3,439 / 注入 716"本来就不是同一把尺子。口径是宿主的，改不了；已在 `tools/table.ts` 的文件头写明它偏在哪，免得后来者拿两个数直接比。
  - **顺带回答过"改成英文会不会更省"：不值得**。框架部分中文 287 → 英文 263 tok，**只省 8%**（o200k；cl100k 省 25%，结论**取决于词表**），而框架只占四个板块的 35%，摊下来省 2.9%～9%；且**长散文中文反而更省**（授权语那句中文 35 / 英文 42）。换英文的代价是这段文本**用户要在界面上审**（`注入实况`），中文才审得动。

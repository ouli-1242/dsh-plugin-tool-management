...
- **注入的元指令精简，并补上场景的因果**（用户 2026-09-23 贴出完整注入全文：「感觉还是不对劲，但说不上来」）。量化之后"不对劲"有两个来源：四个板块合计 716 tok/轮，**内容只占 46%**；而**越短的板块包装占比越高**（场景 87%、MCP 76% —— 三行内容配了 98 tok 元指令）。逐项：
  - **场景段补上因果句**：此前它是四个板块里唯一**没有动作句**的 —— 首句只说"场景是什么"，模型读完不知道该拿它做什么。根因是**场景的语义被窄化了**：它其实是"切换六处开关的运行时模式"（`scene-mode-set` 三步：服务器级/来源级 → 工具级/技能级 → 人设 + 备注），而注入里只呈现了它作为"记忆分组标签"的那一面 —— 四个板块并列，看不出场景是"因"、其余四段是"果"。现在 cue 是动作句（与另三段同形），how 补一句「下面的记忆 / MCP / 技能 / 人设都已经按它筛过」，把四段串起来。代价 **+38 B（≈10 tok/轮）** —— 这一处是**变贵**的，买的是语义清楚。（第一版这里写 +33 B / 8 tok，是用错索引量出来的：复现脚本把 `readIndexSync` 的入参传成了不存在的 `state/` 子目录 → 拿到空索引 → 场景说明丢失，`**「代码」**` 少了一条说明。已按正确索引重测更正 —— 声明过的"实测"就得是实测。）
  ...
  - 四段合计 **715 → 约 656 tok/轮（−59 tok，−8.4%）**。逐项实测（真实 `~/.dsh` 数据 + 正确索引）：场景 251 → 289 B、记忆 1263 → 1045 B、MCP 513 → 458 B、子智能体 834 → 830 B。

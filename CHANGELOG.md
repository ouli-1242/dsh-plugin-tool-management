...
- **注入的内容按一条原则定稿：只给"现在是什么"**（用户 2026-09-23 看到实际渲染后给的原则：「**上下文注入就是当前的情况，目的是让 agent 知道现在的情况，不需要它知道没用的信息，反推更是浪费 token**」）。这条原则是**被实测逼出来的** —— 起点是用户贴出完整注入全文说「感觉还是不对劲，但说不上来」，量化之后"不对劲"有两个来源：四个板块合计 **823 tok/轮**（o200k 真词表），**内容只占 46%**；而**越短的板块包装占比越高**（场景 87%、MCP 76% —— 三行内容配了 98 tok 元指令）。
  ...
  - **记忆段的授权语压缩并提到板块层**（用户同日的两轮裁定：「这段话很多没用信息，浪费 token 和上下文」）。文本 77 → 48 字符（219 → 136 字节），**五个功能一个没动** —— ① 这是什么、② 可能已过期、③ 冲突时以实际情况为准、④ 冲突时以用户当场说的为准、⑤ 无关不必提 + 完整性声明（那半句是**防探测的唯一手段** —— `constants.ts` 里记着"真正防探测的就是它，必须留"）。删掉的只有修饰与重复。位置也从**每个场景块各一遍**改成**整段只说一次**（2 个场景就是 272 B ≈68 tok/轮、占记忆段 22%，而且它夹在 `## 场景：X` 与条目之间、把分组和内容隔开）—— 它管的本来就是整段（「以下就是全部信息」说的是全段）。早前那条「放最顶层会飘在场景之外」的约束**已不成立**：那时板块标题还是 `##`、与场景分组平级；现在板块是 `#` 一级，放在两者之间就是明确的板块级说明。
  ...
  实测（真 BPE 词表 o200k）：**823 → 645 tok/轮（−178，−21.6%）**；cl100k 992 → 761（−23.3%）。逐板块：场景 70 → 44、记忆 365 → 289、MCP 146 → 89、子智能体 242 → 223。

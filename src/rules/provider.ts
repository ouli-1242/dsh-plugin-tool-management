...
export interface RulesProviderFacade {
  /** 失效缓存 + 重投影（写操作后调用）。 */
  invalidate: () => void
  /** 投影当前 preset 可见分组内的 enabled 规则条目。 */
  list: (preset?: string) => Promise<Array<{ name: string; description: string }>>
  /** 按候选名读取规则正文（skill 加载用）。 */
  get: (candidate: any) => Promise<any>
}

/** 注册 agent-scope 规则 provider；返回清理函数（配合 ctx.effect）。 */
export function createRuleProviderRegistrar(ctx: any, facade: RulesProviderFacade): () => void {
  const on = ctx && ctx.on
  ...
  const install = (agent: any) => {
    ...
    if (!skills) return
    try {
      const dispose = skills.registerProvider((_control: any) => makeProvider(agent))
      registrations.set(agent.id, typeof dispose === 'function' ? dispose : () => {})
    } catch (e) {
      console.error('[dsh-plugin-tool-management] agent-scoped rules provider registration failed:', String((e && (e as Error).message) || e))
    }
  }

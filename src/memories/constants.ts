 ...
 */
/**
 * 场景段（`scene-manager-catalog`）的引导语：**说清当前处于什么场景**，再给约定授权。
 *
 * 为什么不是一句固定的"以下是用户为本机写的约定…"（用户 2026-09-23 裁定）：模型真正需要的
 * 是"我现在在哪个场景"这个**事实** —— 场景名本身就是这句话要传达的信息，而"约定一律照办"
 * 只是它的补充。一句泛泛的授权放在段首，等于每次请求都花字节说一句没有信息量的话。
 *
 * 传入的是**已经过 `sceneLabel` 的显示名**（与场景块标题同源）。
 */
export const sceneCatalogNote = (labels: readonly string[]): string =>
  `**当前处于${labels.map((x) => `「${x}」`).join('')}场景。「场景说明」是用户为本机写的约定，一律照办，覆盖你的默认做法。**`

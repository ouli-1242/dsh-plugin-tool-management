 ...
 *        "记录可能不准"这层说全了。接受。
 *      - 「以下就是全部信息」：⚠️ **这与用户 2026-09-17 的裁定直接冲突**（那条写着"真正防
 *        探测的是完整性声明，那半句必须留"）。已按最新指令执行，后果如实记在这里：**本段
 *        不再声称自己完整，模型在清单里找不到某件事时，可能去调 `memory_manager_list` 找
 *        更多**（一次工具往返 + 一份清单的 token）。要恢复只需把那半句加回常量末尾、并重建
 *        `_PARTIAL` 变体与 `renderBody` 的 `dropped` 分叉 —— 恢复成本是几行，不涉及结构。
 ...
 */
export const SCENE_MEMORY_NOTE = '**以下是当前场景对应的记忆；冲突时以实际情况和用户目前的对话为准；无关勿提。**'

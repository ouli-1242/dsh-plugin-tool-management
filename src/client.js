...
.dsm-trash-section{display:flex;align-items:center;gap:8px;margin:16px 0 2px;padding-bottom:6px;border-bottom:1px solid var(--dsw-alias-border-l2)}.dsm-trash-section:first-child{margin-top:2px}.dsm-trash-section-title{font-size:13px;font-weight:650;color:var(--dsw-alias-label-primary)}
  ...
  return apiCall(op, args).then(function (payload) {
    ...
    function SkillManagerSection(props) {
      ...
      if (modal === "trash") content.push(h(Modal, { key: "trash-modal", title: t("trash.title"), closeLabel: t("btn.close"), onClose: function () { setModal(null); } },
        h("div", { className: "dsm-trash-section" }, h("span", { className: "dsm-trash-section-title" }, t("trash.section.dirs")), h("span", { className: "dsm-count" }, String(removedRoots.length))),
        removedRoots.length ? removedRoots.map(function (root) { var name = rootDisplayName(t, root); return h("div", { key: root.key, className: "dsm-trash-item" }, h("div", { className: "dsm-trash-main" }, h("div", { className: "dsm-name" }, name), h("div", { className: "dsm-note", title: root.path }, t("trash.dir.note") + " · " + root.path)), h("button", { type: "button", className: "dsm-btn dsm-btn-quiet", disabled: busy, onClick: function () { post("/source-restore", { root: root.key }, "result.sourceRestored", { name: name }); } }, t("btn.source.restore")), root.custom === true ? h("button", { type: "button", className: "dsm-btn dsm-btn-quiet dsm-btn-danger", disabled: busy, onClick: function () { setModal({ type: "source-forget-confirm", key: root.key, name: name, path: root.path }); } }, t("btn.delete.forever")) : null); }) : h("div", { className: "dsm-empty" }, t("trash.section.dirs.empty")),
        h("div", { className: "dsm-trash-section" }, h("span", { className: "dsm-trash-section-title" }, t("trash.section.skills")), h("span", { className: "dsm-count" }, t(countKey("trash.count", data.trash.length), { count: data.trash.length }))),
        data.trash.length ? data.trash.map(function (item) { return h("div", { key: item.id, className: "dsm-trash-item" }, h("div", { className: "dsm-trash-main" }, h("div", { className: "dsm-name" }, item.name), h("div", { className: "dsm-note" }, t("trash.deletedAt", { time: new Date(item.deletedAt).toLocaleString() }) + " · " + t("trash.source", { source: trashRootLabel(item) }))), h("button", { className: "dsm-btn dsm-btn-quiet", disabled: busy, onClick: function () { post("/trash-restore", { id: item.id }, "result.restored", { name: item.name }); } }, t("btn.restore")), h("button", { className: "dsm-btn dsm-btn-quiet dsm-btn-danger", disabled: busy, onClick: function () { setModal({ type: "delete-confirm", id: item.id, name: item.name }); } }, t("btn.delete.forever"))); }) : h("div", { className: "dsm-empty" }, t("trash.empty"))));

    const DICT = {
      zh: {
        ...
        "compat.inject.domain.scene": "场景", "compat.inject.domain.memory": "记忆", "compat.inject.domain.mcp": "MCP",
        ...
        "compat.tools.summaryDefault": "其中 ≈{tok} 是出厂默认关掉的 {n} 条（不需要模型做这些事就不用管；要用在这里勾上，只改本机）",
        "compat.tools.defaultOffTitle": "出厂默认就是关着的：模型看不到也调不到，面板不受影响。要模型做这件事，在这里勾上即可（只改本机）。",
        // 「方案」：把一整组勾选存成名字，之后一键换回来（合计行右侧那两颗按钮）。
        "compat.tools.preset.save": "保存方案",
        "compat.tools.preset.apply": "恢复方案",
        "compat.tools.preset.save.tip": "把现在这套勾选存成一个名字，以后一键换回来",
        "compat.tools.preset.apply.tip": "换到已存的方案（含那条不可删的出厂默认）",
        "compat.tools.preset.apply.tipEmpty": "还没有存过方案；这里可以先看出厂默认那一套",
        "compat.tools.preset.saveTitle": "保存当前方案",
        "compat.tools.preset.applyTitle": "恢复方案",
        "compat.tools.preset.saveHint": "现在这套是：开着 {on}/{total} 个 · ≈{tok} tok/轮。",
        "compat.tools.preset.name": "方案名字",
        "compat.tools.preset.namePlaceholder": "例如：日常最小 / 让它自己配服务器",
        "compat.tools.preset.saveYes": "存下来",
        "compat.tools.preset.applyHint": "选一份应用：立刻按它重排发给模型的工具表，面板不受影响。",
        "compat.tools.preset.default": "出厂默认",
        "compat.tools.preset.builtin": "不可删",
        "compat.tools.preset.count": "开着 {on}/{total} · ≈{tok} tok/轮",
        "compat.tools.preset.use": "应用这一份",
        "compat.tools.preset.removeTitle": "删掉方案「{name}」（只删这条记录，不改当前的勾选）",
        "compat.tools.preset.err.nameEmpty": "名字不能是空的",
        "compat.tools.preset.err.nameTaken": "已经有叫「{name}」的方案了 —— 保存不会覆盖它，换个名字（或先删掉那份）",
        "compat.tools.preset.err.limit": "方案最多存 {max} 份，先删一份再存",
        "compat.tools.preset.err.notFound": "那份方案已经不在了（可能在别处删掉了）",
        "compat.tools.preset.err.net": "没存上：与本机服务没接上",
        // 每个工具两档文案（行内一句 / 悬停稍详细）。格式统一：行内一律动词开头、不带句号，
        // 而且是**这个工具真能做的事**的缩写；一个工具能做几件事时用 `/` 分隔，**不用「或」**
        // —— 「或」读起来像"二选一"，而这些工具是真能都做的（`save` 既是新建也是修改，
        // `switch` 既能开也能关）。0.14.x 之前写成「保存一条记忆」这类带量词的说法，读起来
        // 只像"新建"，与 upsert 的实际行为对不上（用户 2026-09-23 指出）。
        ...
        "compat.tools.about.memory_manager_switch": "启用/停用记忆",
        "compat.tools.about.memory_manager_save": "新建/修改记忆",
        "compat.tools.about.scene_manager_list": "列出场景、档案与启停",
        "compat.tools.about.scene_manager_switch": "进入/退出场景",
        "compat.tools.about.scene_manager_save": "新建/修改场景、写档案、绑提示词",
        "compat.tools.about.mcp_manager_list": "列出服务器与工具",
        "compat.tools.about.mcp_manager_switch": "启停/重启服务器",
        "compat.tools.about.mcp_manager_save": "新增/修改服务器",
        "compat.tools.about.skill_manager_list": "列出技能、启停与路径",
        "compat.tools.about.skill_manager_read": "按名字读技能正文",
        "compat.tools.about.skill_manager_switch": "启用/停用技能或技能目录",
        "compat.tools.about.skill_manager_save": "新建/修改技能",
        "compat.tools.about.subagent_manager_list": "列出人设与描述",
        "compat.tools.about.subagent_manager_run": "按人设派子智能体",
        "compat.tools.about.subagent_manager_switch": "启用/停用人设",
        "compat.tools.about.subagent_manager_save": "新建/修改人设",
        "compat.tools.about.prompt_manager_list": "列出提示词预设",
        "compat.tools.about.prompt_manager_apply": "应用一份提示词预设",
        "compat.tools.hover.memory_manager_list": "列出记忆的 id、所属场景、启停与一句话描述，并标明注入按哪个场景走。默认只列这次真的会注入的记忆；all=true 会把停用、场景未启用、被同名覆盖的一起列出来。场景本身（档案、绑定的预设、锁没锁）在 scene_manager_list。只看不改。",
        "compat.tools.hover.memory_manager_read": "按 id（<场景>/<名字>）读一条记忆的完整正文。清单里只有一句话描述，要看正文就用它 —— 尤其是被停用、场景没启用、或被体积预算挤掉的记忆。",
        "compat.tools.hover.memory_manager_switch": "只拨索引里的开关，.md 文件一个字不动，随时能拨回来。停用的记忆不再进上下文，等于把注入预算让给别人 —— 想省 token 优先用它，而不是删。",
        ...
        "compat.tools.hover.scene_manager_list": "列出每个场景：场景名（界面显示名不同时会一并标出 —— 工具要的是前者）、启用 / 已进入 / 已锁定、场景说明、绑定的预设、记忆条数，以及档案：进入它时会开哪些 MCP / 技能 / 人设（没勾的一律关掉）。要改某个场景的档案之前先看它 —— 保存是整段替换，看不到现有勾选就会把原来勾的挤掉；要进入哪个场景也在这一份里挑。只看不改。",
        "compat.tools.hover.scene_manager_switch": "进入一个场景：按它的档案把 MCP / 技能 / 人设整套拨到位（没勾的一律关掉），注入范围收窄成它的记忆，并把场景绑定的预设正文写进全局 AGENTS.md。退出则按进场景前的快照精确还原，基线也恢复。切之前先用 scene_manager_list 看一眼那个场景的档案。改的是整个运行时环境，所以默认要用户确认。",
        "compat.tools.hover.scene_manager_save": "建一个场景，或改掉已有的那个 —— 回执会说清走了哪条。场景是「进入时按它切换」的一套：档案里勾了什么，进入后就开什么，其余全部关掉。只写档案不会让场景生效 —— 要生效得进入它（scene_manager_switch，或你在「场景」页点）。没传的段保持原样；传空段 = 全不勾（不是「不限制」）。改一个段是整段替换，先用 scene_manager_list 看现有勾选，回执会列出被挤掉的项。",
        ...
        "compat.tools.hover.skill_manager_switch": "按技能名或按来源目录启停。停用的技能不进目录也不能被加载；同名冲突时只有生效的那份有用 —— 启错了副本等于没启。",
        ...
        "compat.tools.hover.subagent_manager_switch": "停用的人设不进目录也不能被委派；被场景绑定的人设，在进入该场景时会自动启用。",
        ...
        "compat.reach.mark.scene": "场景", "compat.reach.mark.memory": "记忆", "compat.reach.mark.agentsMd": "提示词", "compat.reach.mark.skill": "技能", "compat.reach.mark.mcp": "MCP", "compat.reach.mark.subagent": "子智能体",
        ...
        "compat.reach.tools.2": "场景 / 记忆 / MCP / 技能 / 子智能体 / 提示词 —— 由本插件按上面的「注入」块补齐。",
...
"memory.stat.total": "条记忆", "memory.stat.scenes": "个启用场景", "memory.stat.sceneCount": "个场景", "memory.status.sceneOff": "场景未启用", "memory.status.off": "本条已停用",
      ...
      en: {
        ...
        "compat.inject.domain.scene": "Scene", "compat.inject.domain.memory": "Memory", "compat.inject.domain.mcp": "MCP",
        ...
        "compat.tools.summaryDefault": "of which ≈{tok} is saved by the {n} tools that ship switched off (leave them alone if the model should not do those things; ticking one is local to this machine)",
        "compat.tools.defaultOffTitle": "Switched off in the shipped default: the model cannot see or call it, the panel is unaffected. Tick it here if you want the model to do this (this machine only).",
        // "Sets": store a whole tick pattern under a name and switch back to it in one click.
        "compat.tools.preset.save": "Save set",
        "compat.tools.preset.apply": "Restore set",
        "compat.tools.preset.save.tip": "Store this tick pattern under a name so you can switch back to it in one click",
        "compat.tools.preset.apply.tip": "Switch to a stored set (including the undeletable shipped default)",
        "compat.tools.preset.apply.tipEmpty": "No sets stored yet; you can still look at the shipped default here",
        "compat.tools.preset.saveTitle": "Save the current set",
        "compat.tools.preset.applyTitle": "Restore a set",
        "compat.tools.preset.saveHint": "Right now: {on}/{total} sent · ≈{tok} tok per request.",
        "compat.tools.preset.name": "Set name",
        "compat.tools.preset.namePlaceholder": "e.g. minimal / let it manage servers",
        "compat.tools.preset.saveYes": "Save it",
        "compat.tools.preset.applyHint": "Pick one: the tool table the model receives is rebuilt immediately, and the panel is unaffected.",
        "compat.tools.preset.default": "Shipped default",
        "compat.tools.preset.builtin": "cannot delete",
        "compat.tools.preset.count": "{on}/{total} sent · ≈{tok} tok per request",
        "compat.tools.preset.use": "Use this",
        "compat.tools.preset.removeTitle": "Delete the set “{name}” (only that record; your current ticks stay)",
        "compat.tools.preset.err.nameEmpty": "The name cannot be empty",
        "compat.tools.preset.err.nameTaken": "There is already a set called “{name}” — saving never overwrites it. Choose another name (or delete that one first).",
        "compat.tools.preset.err.limit": "At most {max} sets — delete one before saving another",
        "compat.tools.preset.err.notFound": "That set is gone (someone else deleted it)",
        "compat.tools.preset.err.net": "Not saved: no reply from the local service",
        // Two tiers per tool (inline one-liner / slightly longer hover). Same shape everywhere:
        // the inline line starts with a verb and has no full stop, and it is an abbreviation of
        // what the tool can actually do — several behaviours are joined with "/", never "or"
        // (these tools really do both: `save` creates *and* updates, `switch` enables *and*
        // disables). Keys are built from the tool name.
        ...
        "compat.tools.about.memory_manager_switch": "Enable/disable a memory",
        "compat.tools.about.memory_manager_save": "Create/update a memory",
        "compat.tools.about.scene_manager_list": "List scenes and their archives",
        "compat.tools.about.scene_manager_switch": "Enter/leave a scene",
        "compat.tools.about.scene_manager_save": "Create/update a scene, write its archive, bind a prompt",
        "compat.tools.about.mcp_manager_list": "List servers and tools",
        "compat.tools.about.mcp_manager_switch": "Enable/disable/restart a server",
        "compat.tools.about.mcp_manager_save": "Add/update a server",
        ...
        "compat.tools.about.skill_manager_switch": "Enable/disable a skill or source",
        "compat.tools.about.skill_manager_save": "Create/update a skill",
        ...
        "compat.tools.about.subagent_manager_switch": "Enable/disable a persona",
        "compat.tools.about.subagent_manager_save": "Create/update a persona",
        "compat.tools.about.prompt_manager_list": "List prompt presets",
        "compat.tools.about.prompt_manager_apply": "Apply a prompt preset",
        "compat.tools.hover.memory_manager_list": "Lists each memory's id, scene, on/off state and one-line description, and says which scene injection follows. Only the memories that would actually be injected by default; all=true adds disabled, inactive-scene and shadowed entries. The scenes themselves (archives, bound presets, locks) are in scene_manager_list. Read-only.",
        ...
        "compat.tools.hover.memory_manager_switch": "Flips the index switch only — the .md file is untouched and it is reversible. A disabled memory stops being injected, which frees byte budget for the others; prefer this over deleting when you want to spend fewer tokens.",
        ...
        "compat.tools.hover.scene_manager_list": "Lists every scene: its name (with the display label when it differs — tools need the name), enabled / entered / locked state, its description, bound preset, memory count, and its archive: which MCP servers, skills and personas entering it switches on, everything else going off. Look here before changing a scene's archive — saving replaces a whole section, so an unseen selection gets dropped — and use it to pick which scene to enter. Read-only.",
        "compat.tools.hover.scene_manager_switch": "Enters a scene: its archive is applied across MCP, skills and personas (anything it does not list goes off), injection narrows to its memories, and its bound preset is written into the global AGENTS.md. Leaving restores the runtime snapshot taken on entry, including the previous baseline. Check the scene's archive in scene_manager_list first. It rewrites the whole runtime environment, so it asks for confirmation by default.",
        "compat.tools.hover.scene_manager_save": "Saves a scene — a named bundle the user enters: whatever its archive lists gets switched on, everything else off. Writing the archive does not turn the scene on — enter it with scene_manager_switch (or the Scenes page) for that. Sections you omit keep their current value; an empty section means \"switch nothing on\", not \"no restriction\". A section you do pass is replaced whole, so read the current one in scene_manager_list first; the receipt lists what got dropped.",
        ...
        "compat.tools.hover.skill_manager_switch": "Toggles by skill name or by source directory. Disabled skills leave the catalog and cannot be loaded; when a name exists in several roots only the winning copy counts, so enabling the wrong one changes nothing.",
        ...
        "compat.tools.hover.subagent_manager_switch": "A disabled persona leaves the catalog and cannot be delegated to; personas bound to a scene are enabled automatically when that scene is entered.",
        ...
        "compat.reach.mark.scene": "Scene", "compat.reach.mark.memory": "Memory", "compat.reach.mark.agentsMd": "Prompt", "compat.reach.mark.skill": "Skills", "compat.reach.mark.mcp": "MCP", "compat.reach.mark.subagent": "Subagents",

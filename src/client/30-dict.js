    const DICT = {
      zh: {
        ...
        "compat.tools.about.memory_manager_list": "列出记忆（默认只列会注入的）",
        "compat.tools.about.memory_manager_read": "按 id 读一条记忆的正文",
        "compat.tools.about.memory_manager_set_enabled": "启用或停用一条记忆",
        "compat.tools.about.memory_manager_write": "新建一条记忆",
        "compat.tools.about.memory_manager_update": "改记忆的正文、描述或名字",
        "compat.tools.about.mcp_manager_list": "列出服务器、状态与工具",
        "compat.tools.about.mcp_manager_set_enabled": "启用或停用服务器 / 单个工具",
        "compat.tools.about.mcp_manager_restart": "重启一台服务器",
        "compat.tools.about.mcp_manager_add": "新增一台 MCP 服务器",
        ...
        "compat.tools.hover.memory_manager_write": "在某个场景下建一条记忆（场景要先存在；global 是常驻那个）。记忆会进之后每一轮请求，所以只记从代码、配置、git 推不出来的事；建之前先看清单里有没有已经讲了同一件事的。",
        "compat.tools.hover.memory_manager_update": "改一条现有记忆：正文、描述或改名（旧文件会留在回收站）。没传的字段保持原样；改之前先读一遍、只改要改的那部分，不要把整份重述一遍。",
        "compat.tools.hover.mcp_manager_list": "列出配好的服务器、连没连上、各自有哪些工具，以及你写给这台服务器的备注。曾经连上过、现在连不上的会标出来 —— 那说的是「这台有问题」，不是「没配」。",
        "compat.tools.hover.mcp_manager_set_enabled": "停用整台服务器，或只停它其中一个工具。停用的工具会从模型可见的工具表里消失，执行侧也拦住；重启 DSH 后仍然生效。",
        "compat.tools.hover.mcp_manager_restart": "把服务器停用、等它断开、再按原来的状态启用 —— 改了配置或服务器卡住时用它。过程中这台服务器的工具会短暂消失。",
        "compat.tools.hover.mcp_manager_add": "新增一台服务器（streamable-http 或 stdio），可带 URL、命令、参数、请求头、环境变量与备注，写到项目层或全局层。只在用户明确要求时用。",
...
"memory.stat.total": "条记忆", "memory.stat.scenes": "个启用场景", "memory.stat.sceneCount": "个场景", "memory.status.sceneOff": "场景未启用", "memory.status.off": "本条已停用",
      ...
      en: {
        ...
        "compat.tools.about.memory_manager_list": "List memories (only the injected ones by default)",
        "compat.tools.about.memory_manager_read": "Read one memory's full body by id",
        "compat.tools.about.memory_manager_set_enabled": "Enable or disable one memory",
        "compat.tools.about.memory_manager_write": "Create a memory",
        "compat.tools.about.memory_manager_update": "Update a memory (body, description, name)",
        "compat.tools.about.mcp_manager_list": "List servers, state and tools",
        "compat.tools.about.mcp_manager_set_enabled": "Enable or disable a server or one tool",
        "compat.tools.about.mcp_manager_restart": "Restart a server",
        "compat.tools.about.mcp_manager_add": "Add an MCP server",
        ...
        "compat.tools.hover.memory_manager_write": "Creates a memory in a scene (the scene must exist; global is the always-on one). Memories enter every later request, so record only what cannot be re-derived from code, config or git — and check the list first for an entry that already says this.",
        "compat.tools.hover.memory_manager_update": "Updates an existing memory: body, description, or rename (the old file stays in the trash). Omitted fields are kept as they are; read it first and change only the part that needs changing.",
        "compat.tools.hover.mcp_manager_list": "Lists the configured servers, whether each is connected, which tools it exposes, and the note you wrote for it. Servers that connected before but not now are marked — that means this one is broken, not unconfigured.",
        "compat.tools.hover.mcp_manager_set_enabled": "Disables a whole server, or a single tool on it. A disabled tool disappears from the model-visible tool table and is blocked at execution; this survives a DSH restart.",
        "compat.tools.hover.mcp_manager_restart": "Disables the server, waits for it to drop, then enables it back the way it was — use it after a config change or when a server is stuck. Its tools disappear briefly during the restart.",
        "compat.tools.hover.mcp_manager_add": "Adds a server (streamable-http or stdio) with URL, command, args, headers, env and a note, at project or global level. Only on the user's explicit request.",

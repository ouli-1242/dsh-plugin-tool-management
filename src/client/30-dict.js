    const DICT = {
      zh: {
        ...
        "compat.tools.about.scene_memory_manager_list": "列出记忆与启停",
        "compat.tools.about.scene_memory_manager_read": "按 id 读记忆正文",
        "compat.tools.about.scene_memory_manager_set_enabled": "启用或停用记忆",
        "compat.tools.about.scene_memory_manager_save": "保存一条记忆",
        ...
        "compat.tools.hover.scene_memory_manager_list": "列出记忆的 id、所属场景、启停与一句话描述。默认只列这次真的会注入的；all=true 会把停用、场景未启用、被同名覆盖的一起列出来。只看不改。",
        "compat.tools.hover.scene_memory_manager_read": "按 id（<场景>/<名字>）读一条记忆的完整正文。清单里只有一句话描述，要看正文就用它 —— 尤其是被停用、场景没启用、或被体积预算挤掉的记忆。",
        "compat.tools.hover.scene_memory_manager_set_enabled": "只拨索引里的开关，.md 文件一个字不动，随时能拨回来。停用的记忆不再进上下文，等于把注入预算让给别人 —— 想省 token 优先用它，而不是删。",
        "compat.tools.hover.scene_memory_manager_save": "新建一条记忆，或改掉已经存在的那条同名记忆 —— 回执会说清走了哪条。记忆会进之后每一轮请求，所以只记从代码、配置、git 推不出来的事；建之前先看清单里有没有已经讲了同一件事的。没传的字段保持原样；改名会把旧文件留在回收站。",
...
"memory.stat.total": "条记忆", "memory.stat.scenes": "个启用场景", "memory.stat.sceneCount": "个场景", "memory.status.sceneOff": "场景未启用", "memory.status.off": "本条已停用",
      ...
      en: {
        ...
        "compat.tools.about.scene_memory_manager_list": "List memories and their state",
        "compat.tools.about.scene_memory_manager_read": "Read a memory body by id",
        "compat.tools.about.scene_memory_manager_set_enabled": "Enable or disable a memory",
        "compat.tools.about.scene_memory_manager_save": "Save a memory",
        ...
        "compat.tools.hover.scene_memory_manager_list": "Lists each memory's id, scene, on/off state and one-line description. Only the ones that would actually be injected by default; all=true adds disabled, inactive-scene and shadowed entries. Read-only.",
        "compat.tools.hover.scene_memory_manager_read": "Reads the full body of one memory by id (<scene>/<name>). The list carries a one-line description only — use this for the body, especially for memories that are off, in an inactive scene, or dropped by the byte budget.",
        "compat.tools.hover.scene_memory_manager_set_enabled": "Flips the index switch only — the .md file is untouched and it is reversible. A disabled memory stops being injected, which frees byte budget for the others; prefer this over deleting when you want to spend fewer tokens.",
        "compat.tools.hover.scene_memory_manager_save": "Creates a memory, or updates the existing one with that id — the receipt says which. Memories enter every later request, so record only what cannot be re-derived from code, config or git — and check the list first for an entry that already says this. Omitted fields are kept; a rename leaves the old file in the trash.",

...
export interface AgentsMdService {
  ...
  create(id: string, from?: string): Promise<{ ok: true; id: string } | { ok: false; error: string }>
  update(id: string, content: string): Promise<{ ok: true; id: string } | { ok: false; error: string }>
...
export function createAgentsMdService(_ctx: unknown, deps: AgentsMdDeps): AgentsMdService {
  ...
  async function read(id: string): Promise<{ ok: true; content: string } | { ok: false; error: string }> {
    ...
    if (!PRESET_ID_RE.test(safeId)) return { ok: false, error: '非法 id（仅允许小写字母、数字、连字符）：' + safeId }
    try {
  ...
  async function create(id: string, from?: string): Promise<{ ok: true; id: string } | { ok: false; error: string }> {
    const safeId = String(id || '')
    if (!PRESET_ID_RE.test(safeId)) return { ok: false, error: '非法 id（仅允许小写字母、数字、连字符）：' + safeId }
    ...
    if (srcId) {
      if (!PRESET_ID_RE.test(srcId)) return { ok: false, error: '非法 from id：' + srcId }
  ...
  async function update(id: string, content: string): Promise<{ ok: true; id: string } | { ok: false; error: string }> {
    const safeId = String(id || '')
    if (!PRESET_ID_RE.test(safeId)) return { ok: false, error: '非法 id（仅允许小写字母、数字、连字符）：' + safeId }
    const dir = join(deps.presetsDir, safeId)
  ...
  async function apply(id: string): Promise<{ ok: true; id: string; backedUp: boolean } | { ok: false; error: string }> {
    ...
    try { prev = await readFile(globalPath, 'utf8') } catch { /* 当前不存在，不备份 */ }
    if (prev !== null) {
  ...
  async function getCurrent(): Promise<{ ok: true; content: string; presetId: string | null; exists: boolean } | { ok: false; error: string }> {
    try {
      ...
      for (const e of entries) {
        if (!e.isDirectory() || !PRESET_ID_RE.test(e.name) || e.name === LAST_APPLIED_ID) continue
        let c: string

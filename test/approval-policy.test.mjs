// test/approval-policy.test.mjs —— never 审批策略预检的回归测试（node --test）。
//
// 这个文件锁的是 2026-09-13 19:52 会话实测到的 bug：完全权限（approval=never）
// 会话里 pre-execute 的 never 预检返回 false，模型只收到通用的「用户拒绝」。
...
import { isApprovalNever, neverPolicyHint } from '../lib/approval-policy.js'
...
test('never 预检：会话 fold 为 never（完全权限预设的真实写法）→ true', async () => {
  const { ctx } = await mount('ask')
...
test('never 预检：ask 会话 → false（确认门照常弹出）', async () => {
  const { ctx } = await mount('ask')
  const session = foldSession()
  assert.equal(isApprovalNever(ctx, execFor(session)), false)
})

test('never 预检：全局默认 config.policy=never 且无会话 fold → true；会话 fold 可翻回 ask', async () => {
...
test('never 预检：无 agent/session 时退回 config.policy；两者都没有 → false', async () => {
...
test('never 预检：服务缺失 / 读取抛错 / ctx 无 get → 一律 false（预检不代替审批层）', () => {
...
test('neverPolicyHint：文案给出可行动的下一步（切模式 / 关确认开关）', () => {
  const hint = neverPolicyHint('「运行子代理」的确认无法弹出')
  assert.match(hint, /审批策略为 never/)
  assert.match(hint, /工作区内修改/)
  assert.match(hint, /requireConfirmForModel/)
})

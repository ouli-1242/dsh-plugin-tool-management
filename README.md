...
| 场景记忆自动生效 | 记忆 = `~/.dsh/scene-memory/<场景>/<name>.md`；勾选启用的场景，其目录树内所有 `.md` 正文**自动进入系统提示词**（per-agent `systemPrompt` 段），模型无需任何工具调用，切换后**下一个请求即生效**；**不选场景（根层）= 全局，任何对话都注入** |
| 记忆导入 | 记忆页「导入记忆」：`.md` / `.zip`（可多选、可拖入）；zip 内目录名即场景，裸 `.md` 落到弹窗所选场景（默认留空 = 全局）；同名跳过并列出名单 |
...
- **全局记忆（不选场景）**：场景留空 = 全局，文件放在 `scene-memory/` 根层，**任何对话都注入**（与 `_shared/` 同级恒常生效，不吃场景开关）。适合"放之四海皆准"的偏好与约定。
- **导入记忆**：页头「导入记忆」支持 `.md` 与 `.zip`（可多选、可拖入）。zip 内带目录时**目录名即场景**（`工作/standup.md` → 场景 `工作`）；裸 `.md` 落到弹窗里选的场景——**默认留空 = 全局**。文件按原文落盘，**同名自动跳过并列出名单**（不覆盖既有记忆）。
- **记忆 = 一个 Markdown 文件**：`<场景>/<name>.md`（flat）或 `<场景>/<name>/SKILL.md`（bundle，可带附件）；场景可留空（= 全局）。新建时填场景（从已有场景里挑，或**直接输入新场景名**——目录会自动创建）、名称（= 文件名）、描述与正文，frontmatter 全部可选，缺失时插件自动派生。
...
- **子智能体（人设）**：`~/.dsh/subagents/<人设>.md`，一个文件一个人设——frontmatter 可选（`description` 何时调用，**一句话即可** / `model` 指定模型 / `tools` 工具白名单，缺省自动派生），正文就是人设提示词。页头「导入」支持 `.md` 与 `.zip`（zip 内任意层级的 `.md` 都按文件名导入，**同名自动跳过并列出名单**）。模型用 `subagent_list` 看清单、`subagent_run{agent, task}` 调用：子代理**带人设独立运行**、自动继承当前启用场景的记忆段、只把最终输出回传主模型（≤16 KiB），跑完即弃不进 History。场景档案里绑定「本场景可用哪些人设」（绑定外调用报"人设不可用"）；运行默认弹确认（花的是真 token），设置 `requireConfirmForModelSubagentRun: false` 可关。
...
| 记忆文件（真源） | `~/.dsh/scene-memory/<场景>/<name>.md`（flat）或 `<场景>/<name>/SKILL.md`（bundle）；场景目录名可含中文；**根层（无场景）= 全局，任何对话都注入** |
...
| `subagent_run` 报「spawn provider 不可用」 | 宿主没注册 `spawn` provider，本插件也没能挂载 `@deepseek-ai/dsh-subagent-spawn-in-process`（错误文本里带原始原因）。在宿主 profile 里挂载该包后重启 DSH 即可——本插件不把它写进 `cordis.patch.yml`，以免缺包的宿主整棵树起不来（取舍见该文件注释）。 |
...
npm test             # 构建 + 场景档案冒烟测试（node --test test/archive.test.mjs）
```

> 本项目的验证方式是**直接跑一遍真实行为**（见 `docs/` 下的变更单验收项），而不是断言代码当前怎么实现——
> 后者只是把实现抄一遍，必然通过。唯一例外是场景档案的**纯逻辑 + 引擎状态机**冒烟测试
> （`test/archive.test.mjs`，跑 `lib/` 产物）：它断言的是语义契约（勾=启用、空段可持久化、失败回滚与如实上报），
> 不是实现抄写。真实行为验收仍以浏览器/宿主实测为准，冒烟测试不能替代。

...
| 记忆导入 | 记忆页「导入记忆」：`.md` / `.zip`（可多选、可拖入）；zip 内目录名即场景，裸 `.md` 落到弹窗所选场景（默认留空 = 全局）；zip 内 `<场景>/<名>/SKILL.md` 按 **bundle** 导入（同层文件作附件）；同名跳过并列出名单 |
...
- **导入记忆**：页头「导入记忆」支持 `.md` 与 `.zip`（可多选、可拖入）。zip 内带目录时**目录名即场景**（`工作/standup.md` → 场景 `工作`）；裸 `.md` 落到弹窗里选的场景——**默认留空 = 全局**。zip 内 `<场景>/<名>/SKILL.md` 按 **bundle** 导入（落 `scene-memory/<场景>/<名>/`，**同层文件作附件**；附件名非法 / 空 / 单个 >8 MB / 合计 >16 MB 逐条跳过并回报）。文件按原文落盘，**同名自动跳过并列出名单**（不覆盖既有记忆）。
...
npm test             # 构建 + 全部语义契约测试（node --test test/*.test.mjs）
```

...
import { KEBAB_RE, parseBoolValue, parseSkillDoc, resolveDshHome, unquote } from '../skills/core.js';
import { createRuleProviderRegistrar } from './provider.js';
import { compareAlways, compileAlwaysProjection, DEFAULT_MAX_BYTES, writeAlwaysProjection, } from './project.js';
// ── 常量 ───────────────────────────────────────────────────────────────────
...
// ── 分组名校验 ─────────────────────────────────────────────────────────────
function isValidGroupSegment(segment) {
...
function isValidGroupPath(group) {
...
// ── 侧车文件（索引 / 场景）─────────────────────────────────────────────────
const defaultIndex = () => ({ version: INDEX_VERSION, rules: {}, groups: {} });
...
async function readScenes(stateDir) {
    try {
 ...
 */
function visibleGroupsForPreset(preset, scenes) {
    if (typeof preset === 'string' && scenes.presets[preset]) {
        const groups = new Set(scenes.presets[preset].groups || []);
        if (scenes.includeSharedDefault)
            groups.add(SHARED_GROUP);
        return [...groups];
    }
    return [SHARED_GROUP];
}
...
export function createRulesService(ctx, deps) {
    ...
    // ── 始终层（预算预检 + 输入）────────────────────────────────────────────
    async function alwaysRuleInputs() {
        const snap = await snapshot();
        const items = [];
        for (const rule of snap.rules) {
            if (!rule.always || !rule.enabled || rule.shadowed)
                continue;
    ...
    async function rulesCreate(args) {
        ...
        if (!isValidGroupPath(group))
            return fail('error.rules.invalidGroup', `分组名非法：${group || '(空)'}（仅允许小写字母、数字、连字符，每段 ≤${MAX_GROUP_SEGMENT_LENGTH}）`);
        if (!KEBAB_RE.test(name))
            return fail('error.rules.invalidName', `规则名必须为 kebab-case（小写字母/数字/连字符）：${name}`);
        ...
        if (!index.groups[group])
            index.groups[group] = { order: DEFAULT_GROUP_ORDER, label: group };
        await writeIndex(stateDir, index);
        invalidateSnapshot();
        invalidateProviders();
    ...
    async function rulesUpdate(args) {
        ...
        if (!KEBAB_RE.test(newName))
            return fail('error.rules.invalidName', `规则名必须为 kebab-case：${newName}`);
    ...
    async function listProjected(preset) {
        const scenes = await readScenes(stateDir);
    ...
    async function getProjected(candidate) {

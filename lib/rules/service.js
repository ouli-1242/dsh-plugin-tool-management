...
// ── 侧车文件（索引 / 场景）─────────────────────────────────────────────────
const defaultIndex = () => ({ version: INDEX_VERSION, rules: {}, groups: {}, active: null });
...
function renderSceneMemory(probe, index, maxBytes) {
    ...
    const sceneLabelOf = (scene) => (scene === '' ? '全局' : scene);
...
/** 场景标题：`''` → 全局；其余用目录名（可追溯）。 */
const sceneHeading = (scene) => `## 场景记忆：${scene === '' ? '全局' : scene}`;
/** 单条记忆的标题：显式 description 优先；派生描述与正文重复，改用文件名。 */
const ruleHeading = (f) => (f.descriptionDerived || !f.description ? f.name : f.description);
...
export function createRulesService(ctx, deps) {
    ...
    function sceneRows(snap, index) {
        ...
        for (const group of snap.groups) {
            ...
            if (group.name === scene)
                rec.order = group.order;
            else
                rec.order = Math.min(rec.order, group.order);
        }
    ...
    async function rulesCreate(args) {
        ...
        if (!isValidGroupPath(group))
            return fail('error.rules.invalidGroup', `场景/分组名非法：${group || '(空)'}（非空、≤${MAX_GROUP_SEGMENT_LENGTH} 字符、不含路径分隔符与 < > : " | ? *、不以 . 开头、首尾无空白）`);
    ...
    async function listAttachments(bundleDir) {
        let entries;
        try {
    ...
    async function rulesDetach(args) {
        ...
        if (!isValidGroupSegment(name) || name === BUNDLE_DOC)
            return fail('error.rules.invalidName', `附件名非法：${name || '(空)'}`);
    ...
    async function rulesSetActive(args) {
        ...
        if (args && args.all === true) {
            ...
            for (const item of raw) {
                ...
                if (name === '' || name === SHARED_GROUP)
                    continue; // _shared 恒常，不入集合

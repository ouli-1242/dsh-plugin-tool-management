...
function systemPromptOf(scope) {
    if (!scope)
        return undefined;
    try {
        if (typeof scope.get === 'function')
            return scope.get('systemPrompt');
...
export function createRuleProviderRegistrar(ctx, facade) {
    ...
    const install = (scope, key) => {
        if (!scope || registrations.has(key))
            return;
        ...
        try {
            ...
            console.error('[dsh-plugin-tool-management] scene-memory section registration failed:', String((e && e.message) || e));
        }
    };
    const uninstall = (key) => {
    ...
    if (typeof on === 'function') {
        stopCreated = on('agent/created', (payload) => {
            const agent = payload && payload.agent;
            if (agent && agent.id != null)
                install(agent.ctx, String(agent.id));
        });
        stopDisposed = on('agent/disposed', (payload) => {
            const agent = payload && payload.agent;
            if (agent && agent.id != null)
                uninstall(String(agent.id));
        });
    }
    ...
    return {
        invalidate: () => {
            for (const entry of registrations.values()) {
                try {
                    entry.scope?.emit?.('system-prompt/change');
                }
                catch { /* 宿主无该事件 → 忽略 */ }
            }
        },

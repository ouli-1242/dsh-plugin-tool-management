...
export default {
    ...
    apply(ctx, config) {
        ...
        const presetsService = createPresetsService(ctx, {
            presetsRoot: String(config?.presetsRoot || ''),
        });
        const scenesService = createScenesService(ctx, {
            ...
            invalidateRules: () => {
                const inv = rulesService._invalidate;
                if (typeof inv === 'function')
                    inv();
            },
            listRuleGroups: async () => {
                const r = await rulesService.ops['rules-list']({});
                return (r && r.ok && Array.isArray(r.groups)) ? r.groups.map((g) => ({ key: g.key, label: g.label || g.key, count: g.count })) : [];
            },
        });
        ...
        tools.register(defineTool({
            ...
            async execute(args) {
                ...
                const lines = (r.rules || []).map((x) => ('- ' + x.id + ' [' + (x.always ? '始终' : '按需') + '] ' + (x.enabled ? '已启用' : '已停用') +
            ...
            if (commands && typeof commands.register === 'function') {
                ...
                ctx.effect(() => commands.register({
                    ...
                    handler: async () => {
                        const r = await rulesService.ops['rules-list']({});
                        if (!r || r.ok === false)
                            return { kind: 'error', text: (r && r.error) || '读取规则失败' };
                        const rules = r.rules || [];
                        if (!rules.length)
                            return { kind: 'success', text: '未发现任何规则（~/.dsh/rules 为空）。' };
                        const text = '规则（' + rules.length + '）：\n' + rules.map((rule) => ('- ' + rule.name + ' [' + rule.group + '] ' + (rule.always ? '始终' : '按需') + ' · ' + (rule.enabled ? '已启用' : '已停用'))).join('\n') + '\n（"始终" = 已编译进 AGENTS.md，新会话生效）';
                        return { kind: 'success', text };
                    },

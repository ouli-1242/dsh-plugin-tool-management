...
`
    ...
    module.exports = {
      ...
      apply(ctx) {
        ...
        function ToolsSection() {
          ...
          var page = active === 'mcp' ? React.createElement(MCPPage)
            ...
            : active === 'rules' ? React.createElement(RulesPage)
            : React.createElement(ScenesPage)
          return React.createElement('section', { className: 'dsm-section' },
            React.createElement('div', { className: 'dsm-tabs' },
              ...
              React.createElement('button', { type: 'button', className: 'dsm-tab' + (active === 'scenes' ? ' dsm-tab-active' : ''), onClick: function () { setActive('scenes') } }, 'Scenes'),
      ...
      zh: {
        ...
        "scenes.desc": "把 agent preset（场景）与规则分组绑定：为不同场景指定可见的规则分组，规则按需层会按会话的 preset 自动过滤。",
        "scenes.btn.refresh": "刷新", "scenes.btn.new": "新建场景", "scenes.btn.default": "设为默认", "scenes.btn.copy": "复制为自定义场景", "scenes.btn.remove": "删除", "scenes.btn.create": "创建", "scenes.btn.delete.confirm": "删除",
        "scenes.trust.system": "系统", "scenes.trust.user": "自定义", "scenes.default": "默认", "scenes.unhealthy": "异常",
        "scenes.groups": "可见规则分组", "scenes.groups.empty": "暂无规则分组，请先在 Rules 页创建规则。",
        "scenes.create.title": "新建场景", "scenes.field.from": "来源 preset", "scenes.field.from.placeholder": "选择来源…", "scenes.field.id": "场景 ID", "scenes.field.id.placeholder": "例如 daily-work", "scenes.field.id.hint": "kebab-case：小写字母/数字，以连字符分隔", "scenes.field.name": "显示名", "scenes.field.name.placeholder": "例如 日常办公", "scenes.field.desc": "描述", "scenes.field.desc.placeholder": "这个场景用在什么场合（可选）",
        "scenes.id.invalid": "场景 ID 须为 kebab-case（小写字母/数字，以连字符分隔）",
        "scenes.delete.title": "删除场景？", "scenes.delete.desc": "将删除自定义场景「{name}」及其 preset 目录。系统场景不可删除。",
        "scenes.result.created": "已创建场景：{id}", "scenes.result.removed": "已删除场景：{id}", "scenes.result.default": "已设为默认：{id}", "scenes.result.groups": "已更新可见分组。",
        "scenes.loading": "正在加载场景…", "scenes.empty": "暂无场景。点「新建场景」从系统 preset 开始。",
      ...
      en: {
        ...
        "scenes.desc": "Bind agent presets (scenes) to rule groups: each scene sees only the rule groups it is bound to; the on-demand layer filters by the session preset automatically.",
        "scenes.btn.refresh": "Refresh", "scenes.btn.new": "New scene", "scenes.btn.default": "Set default", "scenes.btn.copy": "Copy as custom", "scenes.btn.remove": "Delete", "scenes.btn.create": "Create", "scenes.btn.delete.confirm": "Delete",
        "scenes.trust.system": "System", "scenes.trust.user": "Custom", "scenes.default": "Default", "scenes.unhealthy": "Unhealthy",
        "scenes.groups": "Visible rule groups", "scenes.groups.empty": "No rule groups yet; create rules on the Rules page first.",
        "scenes.create.title": "New scene", "scenes.field.from": "Source preset", "scenes.field.from.placeholder": "Choose a source…", "scenes.field.id": "Scene ID", "scenes.field.id.placeholder": "e.g. daily-work", "scenes.field.id.hint": "kebab-case: lowercase letters and digits, dash-separated", "scenes.field.name": "Display name", "scenes.field.name.placeholder": "e.g. Daily work", "scenes.field.desc": "Description", "scenes.field.desc.placeholder": "When to use this scene (optional)",
  ...
  return apiCall(op, args).then(function (payload) {
    ...
    function SkillManagerSection(props) {
      ...
      return h("section", { className: "dsm-section" }, content);
        ...
        // ---------- Scenes 页（场景 = agent preset ↔ 规则分组绑定）----------
        function ScenesPage() {
          var state = React.useState({ loading: true, error: null, scenes: [], presets: [], ruleGroups: [] })
          var data = state[0], setData = state[1]
          var ms = React.useState(null)
          var modal = ms[0], setModal = ms[1]
          var ws = React.useState(null)
          var wizard = ws[0], setWizard = ws[1]
          var bs = React.useState(false)
          var busy = bs[0], setBusy = bs[1]
          var rs = React.useState(null)
          var result = rs[0], setResult = rs[1]
          ...
          function refresh(silent) {
            if (!silent) setData(function (prev) { return Object.assign({}, prev, { loading: true, error: null }) })
            apiCall('scene-list', {}).then(function (r) {
              if (r && r.ok) {
                setData({ loading: false, error: null, scenes: r.scenes || [], presets: r.presets || [], ruleGroups: r.ruleGroups || [] })
              } else {
                setData({ loading: false, error: translateError(t, r), scenes: [], presets: [], ruleGroups: [] })
              }
            }).catch(function (e) {
              setData({ loading: false, error: String((e && e.message) || e), scenes: [], presets: [], ruleGroups: [] })
            })
          }
          React.useEffect(function () { refresh() }, [])

          function setGroups(presetId, groups) {
            if (busy) return
            setBusy(true); setResult(null)
            apiCall('scene-set-groups', { presetId: presetId, groups: groups }).then(function (res) {
              setBusy(false)
              if (res && res.ok) { setResult({ ok: true, text: t('scenes.result.groups') }); refresh(true) }
              else setResult({ ok: false, text: translateError(t, res) })
            }).catch(function (e) { setBusy(false); setResult({ ok: false, text: String((e && e.message) || e) }) })
          }
          function toggleGroup(presetId, key) {
            var sc = data.scenes.find(function (s) { return s.presetId === presetId })
            var groups = sc ? sc.groups.slice() : []
            var i = groups.indexOf(key)
            if (i >= 0) groups.splice(i, 1)
            else groups.push(key)
            setGroups(presetId, groups)
          }
          function setDefault(presetId) {
            if (busy) return
            setBusy(true); setResult(null)
            apiCall('scene-set-default', { id: presetId }).then(function (res) {
              setBusy(false)
              if (res && res.ok) { setResult({ ok: true, text: t('scenes.result.default', { id: presetId }) }); refresh(true) }
              else setResult({ ok: false, text: translateError(t, res) })
            }).catch(function (e) { setBusy(false); setResult({ ok: false, text: String((e && e.message) || e) }) })
          }
          function removePreset(presetId) {
            if (busy) return
            setBusy(true); setResult(null)
            apiCall('scene-remove', { id: presetId }).then(function (res) {
              setBusy(false); setModal(null)
              if (res && res.ok) { setResult({ ok: true, text: t('scenes.result.removed', { id: presetId }) }); refresh(true) }
              else setResult({ ok: false, text: translateError(t, res) })
            }).catch(function (e) { setBusy(false); setResult({ ok: false, text: String((e && e.message) || e) }) })
          }
          function openCreate(from) {
            setWizard({ from: from || '', id: '', name: '', description: '', error: null })
            setModal({ type: 'create' })
          }
          function submitCreate() {
            var payload = {
              from: wizard.from || 'standard',
              id: String(wizard.id || '').trim(),
              name: String(wizard.name || '').trim(),
              description: String(wizard.description || '').trim(),
            }
            setBusy(true)
            apiCall('scene-create', payload).then(function (res) {
              setBusy(false)
              if (res && res.ok) {
                setModal(null); setWizard(null)
                setResult({ ok: true, text: t('scenes.result.created', { id: payload.id }) })
                refresh(true)
              } else setWizard(Object.assign({}, wizard, { error: translateError(t, res) }))
            }).catch(function (e) { setBusy(false); setWizard(Object.assign({}, wizard, { error: String((e && e.message) || e) })) })
          }

          var byId = {}
          data.presets.forEach(function (p) { byId[p.id] = p })
          var groupsById = {}
          data.scenes.forEach(function (s) { groupsById[s.presetId] = s.groups })

          function renderScene(sc) {
            var preset = byId[sc.presetId]
            var isSystem = preset && preset.trust === 'system'
            var healthy = !preset || preset.healthy !== false
            var rowGroups = groupsById[sc.presetId] || []
            return React.createElement('div', { key: sc.presetId, className: 'dsm-source' + (healthy ? '' : ' dsm-rule-shadowed') },
              React.createElement('div', { className: 'dsm-source-head' },
                React.createElement('div', { className: 'dsm-source-head-main' },
                  React.createElement('span', { className: 'dsm-source-title' }, (preset && preset.name) || sc.presetId),
                  React.createElement('span', { className: 'dsm-count' }, isSystem ? t('scenes.trust.system') : t('scenes.trust.user')),
                  sc.isDefault ? React.createElement('span', { className: 'dsm-tag dsm-tag-on' }, t('scenes.default')) : null,
                  healthy ? null : React.createElement('span', { className: 'dsm-tag dsm-tag-warn' }, t('scenes.unhealthy'))),
                React.createElement('div', { className: 'dsm-source-actions' },
                  sc.isDefault ? null : React.createElement('button', { type: 'button', className: 'dsm-btn dsm-btn-quiet', disabled: busy, onClick: function () { setDefault(sc.presetId) } }, t('scenes.btn.default')),
                  isSystem
                    ? React.createElement('button', { type: 'button', className: 'dsm-btn dsm-btn-quiet', disabled: busy, onClick: function () { openCreate(sc.presetId) } }, t('scenes.btn.copy'))
                    : React.createElement('button', { type: 'button', className: 'dsm-btn dsm-btn-quiet dsm-btn-danger', disabled: busy, onClick: function () { setModal({ type: 'delete', presetId: sc.presetId, name: (preset && preset.name) || sc.presetId }) } }, t('scenes.btn.remove')))),
              React.createElement('div', { className: 'dsm-source-body' },
                preset && preset.description ? React.createElement('p', { className: 'dsm-note' }, preset.description) : null,
                (!healthy && preset && preset.reason) ? React.createElement('p', { className: 'dsm-feedback dsm-error' }, preset.reason) : null,
                React.createElement('div', { className: 'dsm-scene-groups' },
                  React.createElement('span', { className: 'dsm-label' }, t('scenes.groups')),
                  data.ruleGroups.length ? data.ruleGroups.map(function (g) {
                    var on = rowGroups.indexOf(g.key) >= 0
                    return React.createElement('label', { key: g.key, className: 'dsm-scene-check', style: { marginRight: '12px' } },
                      React.createElement('input', { type: 'checkbox', checked: on, disabled: busy || !healthy, onChange: function () { toggleGroup(sc.presetId, g.key) } }),
                      ' ' + (g.label || g.key))
                  }) : React.createElement('span', { className: 'dsm-help' }, t('scenes.groups.empty')))))
          }

          var invalid = wizard && !KEBAB_RE.test(String(wizard.id || '').trim())

          return React.createElement('section', { className: 'dsm-section' },
            React.createElement('div', { className: 'dsm-head' },
              React.createElement('div', { className: 'dsm-title-block' },
                React.createElement('div', { className: 'dsm-title-row' },
                  React.createElement('h2', { className: 'dsm-title' }, 'Scenes'),
                  React.createElement(VersionBadge, null)),
                React.createElement('p', { className: 'dsm-desc' }, t('scenes.desc'))),
              React.createElement('div', { className: 'dsm-actions' },
                React.createElement('button', { type: 'button', className: 'dsm-btn dsm-btn-secondary', disabled: busy || data.loading, onClick: function () { refresh() } }, t('scenes.btn.refresh')),
                React.createElement('button', { type: 'button', className: 'dsm-btn', disabled: busy, onClick: function () { openCreate('') } }, t('scenes.btn.new')))),
            result ? React.createElement('div', { key: 'result', className: 'dsm-feedback' + (result.ok ? '' : ' dsm-error'), role: 'alert' }, result.text) : null,
            data.error ? React.createElement('div', { key: 'error', className: 'dsm-feedback dsm-error' }, String(data.error)) : null,
            data.loading && !data.scenes.length ? React.createElement('div', { key: 'loading', className: 'dsm-empty' }, t('scenes.loading'))
              : data.scenes.length ? React.createElement('div', { key: 'list', className: 'dsm-sources' }, data.scenes.map(renderScene))
              : React.createElement('div', { key: 'empty', className: 'dsm-empty' }, t('scenes.empty')),
            modal && modal.type === 'delete' ? React.createElement(Modal, { key: 'del', title: t('scenes.delete.title'), closeLabel: t('btn.cancel'), onClose: function () { setModal(null) } },
              React.createElement('p', { className: 'dsm-help' }, t('scenes.delete.desc', { name: modal.name })),
              React.createElement('div', { className: 'dsm-modal-actions' },
                React.createElement('button', { type: 'button', className: 'dsm-btn dsm-btn-secondary', disabled: busy, onClick: function () { setModal(null) } }, t('btn.cancel')),
                React.createElement('button', { type: 'button', className: 'dsm-btn dsm-btn-danger', disabled: busy, onClick: function () { removePreset(modal.presetId) } }, t('scenes.btn.delete.confirm')))) : null,
            wizard && modal && modal.type === 'create' ? React.createElement(Modal, { key: 'create', className: 'dsm-modal-wide', title: t('scenes.create.title'), closeLabel: t('btn.cancel'), onClose: function () { setModal(null) } },
              React.createElement('div', { className: 'dsm-form' },
                React.createElement('label', { className: 'dsm-field' },
                  React.createElement('span', { className: 'dsm-label' }, t('scenes.field.from')),
                  React.createElement(SourceSelect, { options: [{ value: '', label: t('scenes.field.from.placeholder') }].concat(data.presets.map(function (p) { return { value: p.id, label: p.name || p.id } })), value: wizard.from || '', onChange: function (v) { setWizard(Object.assign({}, wizard, { from: v })) } })),
                React.createElement('label', { className: 'dsm-field' },
                  React.createElement('span', { className: 'dsm-label' }, t('scenes.field.id')),
                  React.createElement('input', { className: 'dsm-control' + (invalid ? ' dsm-rule-invalid' : ''), value: wizard.id || '', placeholder: t('scenes.field.id.placeholder'), onChange: function (e) { setWizard(Object.assign({}, wizard, { id: e.target.value })) } }),
                  React.createElement('p', { className: invalid ? 'dsm-rule-hint' : 'dsm-help' }, invalid ? t('scenes.id.invalid') : t('scenes.field.id.hint'))),
                React.createElement('label', { className: 'dsm-field' },
                  React.createElement('span', { className: 'dsm-label' }, t('scenes.field.name')),
                  React.createElement('input', { className: 'dsm-control', value: wizard.name || '', placeholder: t('scenes.field.name.placeholder'), onChange: function (e) { setWizard(Object.assign({}, wizard, { name: e.target.value })) } })),
                React.createElement('label', { className: 'dsm-field' },
                  React.createElement('span', { className: 'dsm-label' }, t('scenes.field.desc')),
                  React.createElement('textarea', { className: 'dsm-control dsm-textarea-sm', value: wizard.description || '', placeholder: t('scenes.field.desc.placeholder'), onChange: function (e) { setWizard(Object.assign({}, wizard, { description: e.target.value })) } })),
                wizard.error ? React.createElement('div', { className: 'dsm-feedback dsm-error', role: 'alert' }, String(wizard.error)) : null),
              React.createElement('div', { className: 'dsm-modal-actions' },
                React.createElement('button', { type: 'button', className: 'dsm-btn dsm-btn-secondary', disabled: busy, onClick: function () { setModal(null) } }, t('btn.cancel')),
                React.createElement('button', { type: 'button', className: 'dsm-btn', disabled: busy || invalid || !String(wizard.id || '').trim(), onClick: submitCreate }, t('scenes.btn.create')))) : null)
        }
        ...
        module.exports._pages = { MCPPage: MCPPage, SkillManagerSection: SkillManagerSection, AgentsMdPage: AgentsMdPage, HistoryPage: HistoryPage, RulesPage: RulesPage, ScenesPage: ScenesPage, t: t }
      },
    }

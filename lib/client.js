...
`
    ...
    module.exports = {
      ...
      zh: {
        ...
        "error.root.readonly": "该来源不允许{action}", "error.root.unknown": "未知技能来源：{root}", "error.root.unsafe": "项目技能目录不安全，拒绝写入：{path}", "error.skill.notFound": "技能不存在: {name}", "error.skill.noFrontmatter": "缺少 frontmatter，无法{action}：{name}", "error.skill.notLoadable": "技能结构不完整，无法{action}: {name}",
...
"memory.stat.total": "条记忆", "memory.stat.scenes": "个启用场景", "memory.stat.sceneCount": "个场景", "memory.status.sceneOff": "场景未启用", "memory.status.off": "本条已停用",
        ...
        "memory.scene.enable": "启用", "memory.scene.enableAll": "全部启用", "memory.scene.off": "未启用", "memory.scene.shared": "常开",
        "memory.scene.global": "全局", "memory.scene.count": "{count} 条记忆", "memory.scene.new": "新建记忆", "memory.scene.pick": "选已有场景，或直接输入新场景名", "memory.scene.browse": "选择已有",
        ...
        "memory.field.group": "场景", "memory.field.group.placeholder": "例如 办公 / 日常 / code-review", "memory.field.group.hint": "即 ~/.dsh/scene-memory/ 下的目录名，中英文皆可",
...
"memory.stat.total": "memories", "memory.stat.scenes": "enabled scenes", "memory.stat.sceneCount": "scenes", "memory.status.sceneOff": "scene off", "memory.status.off": "this memory is off",
        ...
        "memory.scene.enable": "Enabled", "memory.scene.enableAll": "Enable all", "memory.scene.off": "Off", "memory.scene.shared": "always on",
        "memory.scene.global": "Global", "memory.scene.count": "{count} memories", "memory.scene.new": "New memory", "memory.scene.pick": "Pick a scene, or type a new one", "memory.scene.browse": "Pick existing",
        ...
        "memory.field.group": "Scene", "memory.field.group.placeholder": "e.g. office / daily / code-review", "memory.field.group.hint": "Folder name under ~/.dsh/scene-memory/; any Unicode",
    ...
    function SkillManagerSection(props) {
      ...
      return h("section", { className: "dsm-section" }, content);
        ...
        function SceneMemoryPage() {
          ...
          var sceneChoices = order.filter(function (n) { return n !== '' }).map(function (n) { return { value: n, label: sceneLabel(n) } })
          ...
          return React.createElement('section', { className: 'dsm-section' },
            ...
            // 成功反馈走右下角浮层（.dsm-toast，固定定位、不占布局），2.6 秒后自动消失，
            // 见上方 result 自动清理 effect。之前的做法是插进文档流的横幅：每点一次场景
            // 开关就把下面整列卡片顶下去、还一直挂着不走，观感很突兀。
            // 错误反馈仍然留在文档流里且不会自动消失——错误需要一直看得见。
            result && !result.ok ? React.createElement('div', { key: 'result', className: 'dsm-feedback dsm-error', role: 'alert' }, result.text) : null,
            result && result.ok ? React.createElement('div', { key: 'toast', className: 'dsm-toast', role: 'status', 'aria-live': 'polite' }, result.text) : null,
            ...
            editor && modal && modal.type === 'editor' ? React.createElement(Modal, { key: 'editor', className: 'dsm-modal-wide', title: editor.mode === 'create' ? t('memory.create.title') : t('memory.edit.title'), closeLabel: t('btn.cancel'), onClose: closeEditor },
              React.createElement('div', { className: 'dsm-form' },
                ...
                React.createElement('label', { className: 'dsm-field' },
                  ...
                  React.createElement('textarea', { className: 'dsm-control', style: { minHeight: '220px' }, value: editor.body || '', placeholder: t('memory.field.body.placeholder'), onChange: function (e) { setEditor(Object.assign({}, editor, { body: e.target.value })) } })),
                React.createElement('div', { className: 'dsm-field' },

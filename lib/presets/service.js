...
const SYSTEM_PRESET_TEMPLATE = (id, name) => [
    '# agent preset: ' + id,
...
export function createPresetsService(_ctx, deps) {
    const presetsRoot = deps.presetsRoot && deps.presetsRoot.trim() !== '' ? resolve(deps.presetsRoot) : join(resolveDshHome(), '.agent-presets');
    const isSystem = (id) => SYSTEM_PRESETS.some((p) => p.id === id);
    /** 读单个 user preset 的健康与元数据；目录缺失返回 null。 */
    async function readUserPreset(id) {
    ...
    async function get(id) {
        if (isSystem(id))
            return SYSTEM_PRESETS.find((p) => p.id === id) || null;
    ...
    async function create(args) {
        ...
        if (!PRESET_ID_RE.test(id))
            return fail('error.rules.invalidName', 'preset id 需为 kebab-case（小写字母、数字、连字符）。');
        if (await get(id))
            return fail('error.scenes.duplicate', 'preset「' + id + '」已存在。');
        const src = await get(from);
        if (!src)
            return fail('error.rules.notFound', '来源 preset「' + from + '」不存在。');
        const dir = join(presetsRoot, id);
        try {

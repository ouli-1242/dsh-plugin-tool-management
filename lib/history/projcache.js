...
import { SessionProjectionCache, checkpointIdentity, checkpointRecord, projectionCacheDomainSpec } from "@deepseek-ai/dsh-session-projection-cache";
...
for (const field of ["isSeeded", "inheritedEventCount"]) {
    ...
    if (Object.hasOwn(checkpointIdentity.shape, field))
        legacyOptionalIdentityFields[field] = true;
}
...
class SafeSessionTable {
    ...
    get(sessionId) {
        ...
        if (stored === void 0 || stored.sessionId !== sessionId)
            return void 0;
    ...
    *entries() {
        for (const [key, stored] of this.table.entries()) {
            if (projectionCacheStorageKey(stored.sessionId) !== key)
                continue;
...
function normalizeLegacyRecord(record) {
    if (record === null || typeof record !== "object" || record.identity === null || typeof record.identity !== "object")
        return void 0;
    ...
    if (identity.isSeeded !== void 0 && identity.isSeeded !== false)
        return void 0;
    if (inheritedEventCount !== void 0 && inheritedEventCount !== 0)
        return void 0;
    return {
        ...record,
        identity: {
...
async function importPreviousProjectionCache(ctx, target) {
    ...
    let imported = 0;
    if (legacySafeV2.opened)
        imported += await importMissingRecords(ctx, target, legacySafeV2.records, "archive-manager v2");
    if (legacySafe.opened)
        imported += await importMissingRecords(ctx, target, legacySafe.records, "archive-manager v1");
    if (!sameSpec) {
        const current = await readSourceRecords(ctx, projectionCacheDomainSpec, "current");
        if (current.opened)
            imported += await importMissingRecords(ctx, target, current.records, "current");
    }
    if (legacy.opened)
        imported += await importMissingRecords(ctx, target, legacy.records, "legacy v3");
    return imported;
}
...
var ArchiveProjectionCache = class extends SessionProjectionCache {
    ...
    async putSoft(id, identity, rows, what) {
        ...
        if (typeof upstream === "function")
            return upstream.call(this, id, identity, rows, what);
        try {
...
//#endregion
export { ArchiveProjectionCache, ArchiveProjectionCache as default, SafeSessionTable, importPreviousProjectionCache, legacyProjectionCacheDomainSpec, legacySafeProjectionCacheDomainSpec, legacySafeV2ProjectionCacheDomainSpec, projectionCacheStorageKey, safeProjectionCacheDomainSpec };

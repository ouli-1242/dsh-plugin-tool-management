...
function jsonlSessionDirectory(persistence, header, location) {
    ...
    if (!/^session(?:\.v[1-9][0-9]*)?\.jsonl(?:\.zstd)?$/.test(basename(location.path)))
        return;
    ...
    if (location.path !== join(directory, basename(location.path)))
        return;
    return directory;
}
...
const sessionIdSchema = {
    parse(value) {
        if (typeof value !== "string" || value.length === 0)
            throw new TypeError(`sessionId must be a non-empty string, got ${String(value)}`);
        return value;
    },
};
const workspaceIdSchema = {
    parse(value) {
        if (typeof value !== "string" || value.length === 0)
            throw new TypeError(`workspaceId must be a non-empty string, got ${String(value)}`);
        return value;
    },
};
...
const archivedBatchTargetSchema = {
    parse(value) {
        ...
        if (value.scope === "all" || value.scope === "ungrouped")
            return value;
        if (value.scope === "workspace" &&
            typeof value.workspaceId === "string" &&
            value.workspaceId.length > 0)
            return value;
        if (value.scope === "sessions" &&
...
const unarchivedBatchSchema = {
    parse(value) {
        ...
        if (!Array.isArray(value.archivedSessionIds) ||
            ...
            throw new TypeError("archivedSessionIds must be a string array");
        if (!Array.isArray(value.unarchivedSessionIds) ||
...
const archivedWorkspaceBatchSchema = {
    parse(value) {
        ...
        if (!Array.isArray(value.archivedSessionIds) ||
            value.archivedSessionIds.some((id) => typeof id !== "string"))
            throw new TypeError("archivedSessionIds must be a string array");
        if (!Array.isArray(value.archivedSessionIdsAdded) ||
...
const deletedBatchSchema = {
    parse(value) {
        ...
        if (!Array.isArray(value.failures) ||
            value.failures.some((failure) => typeof failure !== "object" ||
                failure === null ||
...
var ArchiveWorkspaceRegistry = class extends WorkspaceRegistry {
    ...
    async saveArchivedAt() {
        try {
            const obj = {};
            for (const [k, v] of this.archivedAtMap)
                obj[k] = v;
    ...
    async archivedSessionDetails() {
        ...
        for (const sessionId of [...new Set(this.requireState().archivedSessionIds)]) {
            ...
            try {
                ...
                this.ctx.logger?.warn?.(`history: could not read details for archived session "${sessionId}": ${String(error)}`);
            }
            items.push(entry);
        }
        return { items };
    ...
    async repairArchivedProjection(header) {
        ...
        try {
            ...
            if (cache.cachedSnapshot(meta, inheritedEventCount) !== void 0)
                return false;
    ...
    async unarchiveSession(sessionId) {
        return this.enqueueOperation(async () => {
            ...
            await this.ensureArchivedAtLoaded();
            if (this.archivedAtMap.delete(sessionId))
                await this.saveArchivedAt();
    ...
    clearTombstone(sessionId) {
        ...
        if (idx !== -1)
            this.deletedSessionOrder.splice(idx, 1);
    ...
    forgetIndexedSession(sessionId) {
        for (const evicted of trackTombstone(this.deletedSessionIds, this.deletedSessionOrder, sessionId, this.deletedSessionTombstoneLimit))
    ...
    async coldReuseKnown(id) {
        ...
        if (listed.createdAt === deletedIdentity.createdAt &&
    ...
    async indexHeader(header) {
        if (this.deletedSessionIds.has(header.id))
            return;
    ...
    async indexHeaders(items) {
        for (const item of items)
            await this.indexHeader(item.header ?? item);
    }
                ...
                for (const path of [dirname(directory), directory]) {
                    ...
                    try {
                        ...
                        if (error?.code === "ENOENT")
                            continue; // Already removed on an earlier attempt.
                        throw error;
                    }
        ...
        try {
            ...
            if (sessions !== void 0)
                for (const session of sessions.list()) {
                    if (session.header.parentSession === sessionId &&
            ...
            for (const childId of descendants) {
                try {
                    if (!(await this.sessionKnown(childId)))
                        continue;
    ...
    async cleanSpill(sessionId) {
        try {
            ...
            if (spill === void 0 || typeof spill.root !== "string")
                return;

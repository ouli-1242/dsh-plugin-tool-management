...
export function userRoots() {
    return [
        ...
        {
            key: "codex",
            path: join(process.env.DSH_CODEX_HOME || join(homedir(), ".codex"), "skills"),
            label: "Codex",
            mutable: false,
            toggleable: true,
            native: false,
        ...
        {
            key: "claude",
            path: join(process.env.DSH_CLAUDE_HOME || join(homedir(), ".claude"), "skills"),
            label: "Claude",
            mutable: false,
            toggleable: true,
            native: false,
...
export function customRootsFromState(stateValue) {
    const list = stateValue && Array.isArray(stateValue.customRoots)
    ...
    for (const item of list) {
        ...
        if (!isAbsolute(path) || !CUSTOM_ROOT_KEY_RE.test(key))
            continue;
        if (key !== customRootKey(path))
            continue;
        out.push({
            key,
            path,
...
export async function addCustomRoot(inputPath, label, log) {
    ...
    if (log)
        log("custom-add", `添加自定义技能目录 ${canonical} (${key})`);
...
export async function removeCustomRoot(key, log) {
    ...
    await writeManagerState(current.state);
    if (log)
        log("custom-remove", `移除自定义技能目录 ${target.path} (${clean})`);
...
export async function projectRoots(projectCwds = [], diagnostics) {
    ...
    for (const project of [...projects.values()].sort((a, b) => a.root.localeCompare(b.root))) {
        ...
        for (const candidate of candidates) {
            if (await projectSourceSafe(candidate))
...
async function checkedWritableRootDefinition(root) {
    ...
    if (!definition || definition.scope !== "project")
        return definition;
...
async function overlapsUserSkillRoot(path) {
    ...
    for (const root of userRoots()) {
        ...
        if (isSameOrDescendant(candidate, userPath) ||
                ...
                if (m) {
                    ...
                    if (block) {
                        ...
                        const indentation = content
                            ...
                            .reduce((min, line) => Math.min(min, (/^\s*/.exec(line) || [""])[0].length), Infinity);
                        const normalized = content.map((line) => Number.isFinite(indentation)
...
export async function renameWithRetry(source, destination, options = {}) {
    ...
    for (let attempt = 1;; attempt++) {
        try {
            ...
            if (!TRANSIENT_RENAME_CODES.has(error && error.code) ||
                attempt >= maxAttempts)
                throw error;
...
function entryOf(name, kind, docPath, doc) {
    ...
    const invocationPolicyValid = (doc.map["disable-model-invocation"] === undefined ||
...
export async function scanEntries(root, options = {}) {
    ...
    if (definition && !definition.mutable) {
        ...
        for (const entry of discovered.entries) {
            try {
                entries.push({
                    ...entryOf(entry.name, entry.kind, entry.docPath, parseSkillDoc(await fs.readFile(entry.realDocPath, "utf8"))),
                    ...entry,
                });
            }
            catch {
    ...
    for (const it of items) {
        try {
            ...
            if (it.isDirectory() && entryPath(root, it.name) !== null) {
                ...
                if (!(await isInsideResolvedRoot(rootReal, docPath)))
                    continue;
                const doc = options.metadataOnly
                    ? { map: {}, hasFrontmatter: false }
                    : parseSkillDoc(await fs.readFile(docPath, "utf8"));
...
async function scanDeduplicatedRoots(roots = userRoots(), options = {}) {
    ...
    for (const group of groups.values()) {
        group.sort((left, right) => Number(left.entry.linked) - Number(right.entry.linked) ||
...
async function visibleEntryForRoot(root, name) {
    ...
    const roots = root.scope === "project" || CUSTOM_ROOT_KEY_RE.test(root.key || "")
        ? [root]
        : userRoots();
...
function validPolicyLists(value, allowMissing = false) {
    ...
    for (const [key, list] of Object.entries(value)) {
        if (!userRoots().some((root) => root.key === key) &&
...
function validManagerStateDocument(value) {
    ...
    if (!value.sources ||
        ...
        return false;
    if (!validPolicyLists(value.disabledSkills) ||
    ...
    for (const root of userRoots()) {
        if (root.key === "dsh")
            continue;
        if (typeof value.sources[root.key] !== "boolean")
            return false;
    ...
    for (const field of ["disabledSkills", "enabledSkills"]) {
        ...
        if (source && typeof source === "object" && !Array.isArray(source)) {
            for (const [key, list] of Object.entries(source)) {
                if (!PROJECT_ROOT_KEY_RE.test(key) && !customKeys.has(key))
                    continue;
                if (!Array.isArray(list))
                    continue;
...
function normalizeCustomRoots(value) {
    ...
    for (const item of value) {
        ...
        if (!isAbsolute(path) || !CUSTOM_ROOT_KEY_RE.test(key))
            continue;
        if (key !== customRootKey(path) || seen.has(key))
            continue;
        seen.add(key);
        out.push(typeof item.label === "string" && item.label.trim()
...
function managerSkillOverride(policy, rootKey, name, policyAliases = []) {
    if ((policy.enabledSkills[rootKey] || []).includes(name))
        return true;
    if ((policy.disabledSkills[rootKey] || []).includes(name))
        return false;
    let inheritedEnable = false;
    for (const alias of policyAliases) {
        if (alias.rootKey === rootKey && alias.name === name)
            continue;
...
function effectiveSkillPolicy(policyResult, root, entry) {
    ...
    const sourceEnabled = root.key === "dsh" ||
        root.scope === "project" ||
...
async function checkedPolicyRootDefinition(root) {
    ...
    if (definition.scope !== "project") {
        ...
        if (CUSTOM_ROOT_KEY_RE.test(definition.key || ""))
            return definition;
    ...
    const validProjectRoot = (definition.kind === "project-dsh" ||
        ...
        resolve(definition.path) ===
            resolve(join(definition.projectRoot, definition.kind === "project-dsh" ? ".dsh" : ".agents", "skills"));
    if (!validProjectRoot)
        return null;
...
async function setPolicySkillEnabled(root, name, enabled, log) {
    ...
    await writeManagerState(current.state);
    if (log)
        log(enabled ? "policy-enable" : "policy-disable", `${enabled ? "启用" : "停用"} ${definition.key}/${name}（本地策略，源文件不变）`);
...
async function readTrashMetadata(id) {
    if (entryPath(trashRootPath(), id) === null)
        return null;
    try {
...
async function restoreRootDefinition(metadata, options = {}) {
    ...
    if (metadata.root.scope === "user" && metadata.root.key === "dsh")
        return rootByKey("dsh");
    if (metadata.root.scope !== "project" ||
    ...
    const root = roots.find((item) => item.key === metadata.root.key &&
    ...
    try {
        for (const target of targets) {
            ...
            if (transferred.cleanupError && log)
                log("trash-source-warning", `技能已跨盘移入回收站，但源盘隐藏副本等待后续清理: ${transferred.quarantine}（${transferred.cleanupError.message || transferred.cleanupError}）`);
        ...
        if (published.cleanupError && log)
            log("trash-stage-warning", `回收站已发布，但临时目录等待后续清理: ${stage}（${published.cleanupError.message || published.cleanupError}）`);
        if (log)
            log("trash", `移到回收站 ${name} -> ${finalPath}`);
        ...
        for (const item of moved.reverse()) {
            try {
                ...
                rollbackFailures.push({
                    ...
                    error: String(rollbackError && rollbackError.message
        ...
        if (rollbackFailures.length) {
            ...
            throw codedError(`${causeText}；移入回收站回滚失败，未恢复内容保留在: ${stage}`, "error.trash.rollbackFailed", { path: stage, error: causeText });
        }
...
export async function restoreTrash(id, log, options = {}) {
    ...
    try {
        for (const fileName of metadata.entries) {
            ...
            if (!isSameOrDescendant(itemRoot, source) ||
        ...
        if (log)
            log("restore", `从回收站恢复 ${metadata.name} -> ${root}`);
        return { id, name: metadata.name, root: trashRootMetadata(definition) };
    }
    catch (error) {
        for (const item of moved.reverse())
            await movePathWithFallback(item.destination, item.source, options.renameOptions).catch(() => undefined);
        throw error;
    }
}
export async function permanentlyDeleteTrash(id, log) {
    ...
    if (log)
        log("trash-delete", `永久删除回收站条目 ${metadata.name} (${id})`);
...
async function collectCandidates(dir) {
    ...
    for (const it of items) {
        if (it.isSymbolicLink())
            throw codedError(`不支持包含符号链接的 skill 来源: ${join(dir, it.name)}`, "error.source.symlink", { path: join(dir, it.name) });
        if (it.isDirectory()) {
            ...
            if (st && st.isSymbolicLink())
                throw codedError(`不支持包含符号链接的 skill 来源: ${sk}`, "error.source.symlink", { path: sk });
            if (st && st.isFile()) {
                out.push({
...
async function assertNoSymbolicLinks(source) {
    ...
    while (pending.length) {
        ...
        if (current.depth > MAX_SOURCE_DEPTH)
            throw codedError(`skill 来源目录层级超过 ${MAX_SOURCE_DEPTH} 层: ${source}`, "error.source.tooDeep", { depth: MAX_SOURCE_DEPTH, path: source });
        const st = await fs.lstat(current.path);
        if (st.isSymbolicLink())
            throw codedError(`不支持包含符号链接的 skill 来源: ${current.path}`, "error.source.symlink", { path: current.path });
        if (!st.isDirectory())
            continue;
        ...
        for (const item of items) {
            ...
            if (item.isSymbolicLink())
                throw codedError(`不支持包含符号链接的 skill 来源: ${path}`, "error.source.symlink", { path });
            if (item.isDirectory())
...
async function replaceWithCopy(source, dest, isDir, existing = []) {
    ...
    try {
        ...
        if (rollbackFailures.length) {
            const causeText = String(error && error.message ? error.message : error);
            throw codedError(`${causeText}；覆盖导入回滚失败，备份保留在: ${rollbackFailures.join("、")}`, "error.import.rollbackFailed", { path: rollbackFailures.join("; "), error: causeText });
        }
        throw error;
    }
    const warnings = [];
    ...
    if (conflict === "overwrite") {
        for (const c of conflicts) {
            try {
                ...
                if (log)
                    log("import-overwrite", `覆盖导入 ${c.source} -> ${dest}`);
            }
            catch (e) {
        ...
        for (const c of conflicts)
            skipped.push({ name: c.name, source: c.source });
    }
    if (failed.length && imported.length === 0)
        return failureResult();
...
function normalizeUploadPath(input) {
    ...
    const parts = raw
        .split("/")
        .filter((part, index, all) => directory && index === all.length - 1 ? false : true);
    if (!parts.length ||
...
async function writeUploadedEntries(contentRoot, entries) {
    ...
    if (entries.length > MAX_UPLOAD_ENTRIES)
        throw codedError(`上传条目超过 ${MAX_UPLOAD_ENTRIES} 个`, "error.upload.tooMany", { limit: MAX_UPLOAD_ENTRIES });
    let total = 0;
    const seen = new Set();
    for (const entry of entries) {
        ...
        if (seen.has(key))
            throw codedError(`上传内容包含重复路径: ${normalized.path}`, "error.upload.duplicate", { path: normalized.path });
        seen.add(key);
        ...
        if (total > MAX_UPLOAD_TOTAL_BYTES)
            throw codedError(`上传内容总大小超过 ${MAX_UPLOAD_TOTAL_BYTES} 字节`, "error.upload.tooLarge", { limit: MAX_UPLOAD_TOTAL_BYTES });
        const target = join(contentRoot, ...normalized.path.split("/"));
        if (!isSameOrDescendant(contentRoot, target))
            throw codedError(`上传条目路径非法: ${normalized.path}`, "error.upload.path", { path: normalized.path });
        ...
        files = unzipSync(archive, {
            filter(info) {
                ...
                if (count > MAX_UPLOAD_ENTRIES)
                    throw codedError(`ZIP 条目超过 ${MAX_UPLOAD_ENTRIES} 个`, "error.upload.tooMany", { limit: MAX_UPLOAD_ENTRIES });
                if (!normalized.directory && info.originalSize > MAX_UPLOAD_ENTRY_BYTES)
                    throw codedError(`ZIP 条目过大: ${normalized.path}`, "error.upload.tooLarge", { limit: MAX_UPLOAD_ENTRY_BYTES });
                total += normalized.directory ? 0 : info.originalSize;
                if (total > MAX_UPLOAD_TOTAL_BYTES)
                    throw codedError(`ZIP 解压后总大小超过 ${MAX_UPLOAD_TOTAL_BYTES} 字节`, "error.upload.tooLarge", { limit: MAX_UPLOAD_TOTAL_BYTES });
                return !normalized.directory;
            },
        });
    }
    catch (error) {
        if (error && /^error\./.test(String(error.code || "")))
            throw error;
...
async function prepareUploadedSource(sessionRoot, input) {
    ...
    if (input && input.zip !== undefined)
        ...
        await writeUploadedEntries(contentRoot, input && input.entries);
    const rootSkill = join(contentRoot, "SKILL.md");
    const rootSkillStat = await lstatOrNull(rootSkill);
    if (!rootSkillStat || !rootSkillStat.isFile())
        return contentRoot;
...
export async function createSkill(input, log, options = {}) {
    ...
    if (log)
        log("create", `创建 ${join(target, "SKILL.md")}`);
...
export async function state(options = {}) {
    ...
    for (const root of [...scoped, ...user]) {
        ...
        for (const e of entries) {
            ...
            const managerEnabled = policyResult.writable !== false &&
    ...
    for (const root of result.roots.filter((item) => item.scope === "project")) {
        if (seenProjects.has(root.projectRoot))
            continue;

...
The model can manage everything above via 17 tools (`mcp_manager_*` / `skill_manager_*` / `prompt_manager_*` / `scene_memory_manager_*` / `subagent_manager_*`); the four `*_save` tools are upserts (create, or update the one that already has that name); scripts use `POST /dsh-plugin-tool-management/api` (`{op, args}` protocol).

Those 17 tool definitions total roughly 3,090 tokens, and the tool table is **sent with every single request**. Switch off the ones you never use in the **"Model tool table"** block on the **Host tab** (one by one, or a whole domain at once) — a switched-off tool is not sent at all, which is the only thing that genuinely saves tokens. The model then cannot call it, but **the panel is unaffected** (the 118 ops and the tool table are independent). Nothing is switched off by default.
...
- **Run and discard**: `subagent_manager_run` runs with the persona, returns only the result, never enters History. A child's context is **persona + task** only: memories are not injected (the child can read them with `scene_memory_manager_*`, and relevant facts belong in the `task`). Scenes can bind which personas are available.

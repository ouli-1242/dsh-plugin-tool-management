...
npm test             # build + i18n check + all semantic-contract tests (node --test test/*.test.mjs, 11 groups / 82 cases)
```
...
> **semantic-contract** tests (`npm test`, run against the built `lib/`, 82 cases):
...
> always clipped to the cap). They

...
- **场景和记忆分家了**：以前它们挤在一个工具族里、也挤在一条注入里，现在各自独立 —— 工具上是两族（记忆 4 条 + 场景 1 条），注入上是两段（「场景」讲当前启用的是哪个、它是什么；「记忆」讲各场景下的条目）。好处有两个：**能单独关掉记忆那一半**（省字节）而保留场景说明；模型终于**能建场景、能给场景配 MCP / 技能 / 子智能体、能绑提示词**了（此前只能看着）。注意写场景档案**不会**让场景生效 —— 要不要用它，仍然由你在「场景」页决定。另外，场景那一段只在**启用了具体场景**时才会出现：当前就是「全局」默认状态时它不注入（说"当前处于全局"是废话，不该占上下文）。场景那一段现在只剩一行内容：`「代码」—— 写代码`（场景名 + 你写的描述；没填描述就只显示场景名），场景名不再重复三遍，也不再把它当成"要照办的约定" —— 它就是一句"这个场景是干什么的"。
...
- **注入里的套话全削了，只留"现在是什么"**：把四次实际注入摊开量了一遍 —— 四个板块 **823 token/轮**（真词表），**真正的内容只占 46%**，其余都是"我是谁、什么时候看我、以我为准"这类包装；越短的板块越离谱（场景 87%、MCP 76%，三行内容配了 98 token 的套话）。后来你给了一条原则，就照它把剩下的也砍了：**上下文注入就是当前的情况，目的是让模型知道现在的情况，不需要它知道没用的信息，反推更是浪费 token**。于是：
  ...
  现在四个板块的框架**只剩三样**：标题、一句补充说明（只有 MCP 和子智能体有）、"以本份为准"那句。四块合计 **823 → 645 token/轮（省了 178，−21.6%）**。顺带发现一件事：**一直用来数 token 的那个口径对中文不准** —— 它按"字符数 ÷ 4"算，对英文高估约 10%、对中文低估约 13%（工具表 18 条真值约 3,137，注入四块真值 823）。所以"工具表 3,439 / 注入 716"这两个数本来就不是同一把尺子；已在 `src/tools/table.ts` 的文件头写明，免得后来者拿两个数直接比。也顺手量过"改成英文会不会更省"：**不值得** —— 框架部分只省 8%（换个老词表能省 25%，所以结论取决于词表），而长句中文反而更省；更关键的是这段文本**你要在「注入实况」里审**，中文才审得动。

- **记忆条目上那句说明瘦了一圈**：从"以下是用户为本机写的记录，可能已过期 —— …"压成 `本机记录，可能已过期：冲突时以实际情况和用户当场说的为准；无关不必提。以下就是全部信息。`（少 38%）。意思一个没少：它是什么、可能过期、和实际情况或你当场说的冲突时听后两者的、无关的不必提，以及"就这些了"（这半句是**防模型去翻工具找更多**的唯一手段，不能删）。删掉的只是重复 —— 板块标题已经说过"这是本机的记忆"了。

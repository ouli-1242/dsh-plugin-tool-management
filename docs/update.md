...
| 版本                         | 发布       | 主题                                                                                                                                                                      |
| ---------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [0.14.0](#0140---2026-09-23) | 2026-09-23 | 场景与记忆分家（共 20 条工具）+ 模型能进入 / 退出场景 + 场景档案对模型可见、改档案会报出被挤掉的项 + 注入只写"现在是什么"（省 21.9%）+ 十五条工具出厂默认关（每轮实发 5 条 ≈1,090）+ 工具表可存方案一键换回 + 人设思考强度 + 改 MCP 配置不碰明文凭据 + 提示词「生效中」修好 |
...
- **场景和记忆分家了**：工具上是两族（记忆 4 条、场景 3 条 —— `_list` 查、`_switch` 进入 / 退出、`_save` 建与改），注入上是两段（「场景」讲当前启用的是哪个、它是什么；「记忆」讲各场景下的条目）。好处有两个：能**单独关掉记忆段**而保留场景说明；模型终于能查场景、配场景、进 / 退场景。注意写档案**不会**让场景生效 —— 生效要进入它。

- **模型能进入 / 退出场景**（`scene_manager_switch`）：以前它看得到场景、却只能说一句"请你到界面上点"。现在你说"切到写作场景"，它就能按该场景的档案把 MCP / 技能 / 人设整套拨到位、注入收窄成这个场景的记忆、把绑定的提示词写进 `AGENTS.md`；退出按进场景前的快照还原。它改的是整个运行时环境（和你在界面上点那一下同一条路径），所以**默认要你确认**。

- **改场景档案之前，模型能先看到现在勾了什么**：`scene_manager_list` 列出每个场景的档案、绑的预设、锁没锁、几条记忆。保存是**整段替换**，以前它想"再加一台服务器"就会把你原来勾的**静默挤掉**、回执还写着成功；现在回执会列出被挤掉的项（`mcp 4 项 → 2 项（-B、-C）`）。

- **注入的套话全削了，只留"现在是什么"**：把四次实际注入摊开量过 —— 四个板块 823 token/轮，**真正的内容只占 46%**，其余是"我是谁、什么时候看我、以我为准"。于是六个板块开头那句加粗的"什么时候该想起我"全删（它们是指令，不是当前情况）、机制解释删、四种措辞的权威声明并成一句。现在 **643 token/轮（省 180，−21.9%）**。层级也理清：板块 `#` / 场景 `##` / 条目 `-`，一眼看得出主次。

- **工具改名并合并：还是 20 条，但每族只管一件事**。五个「新建」并成 `_save`（有则改、无则建），六个域的开关统一叫 `_switch`，场景自成一族。**旧名不留别名**（别名要常驻每轮请求）。你以前"关掉了某条工具"的设置会自动搬到新名字上，包括存下的方案。

- **十五条工具出厂就是关着的**：每轮真正发出去的只剩 **5 条 ≈1,090** token（整表 ≈3,769）。关的是三个「保存」、提示词两条、三个目录查询、`memory_manager_read`、场景三条与三个启停开关 —— 判据是**这些信息或动作常态下由注入段和面板承担**，平时用不到模型自己伸手。**`mcp_manager_switch` 留着**（"这台先给我关了"最常用）。代价摆明：注入的目录只列当前可用的，所以"没开的服务器 / 被同名覆盖的技能 / 没进上下文的记忆正文"在会话里查不到，模型也不再替你拨那三个启停。这条默认**只在你从没动过那一页时生效**，存过的选择不会被更新盖掉；「兼容」页可逐条打开。

- **工具表的开关能成批存下来、一键换回**：「模型工具表」合计行右侧多了**「保存方案」/「恢复方案」**。存的时候起个名字，以后点一下就整套换回，不用挨个点二十个框。**名字不能重复**（保存永不覆盖已有那份）。清单第一条永远是**出厂默认**，它不写在文件里、也就删不掉；自己存的方案可以在同一个弹窗里删。每行都写着"开着几条 · 一轮大约多少 token"。

- **MCP 与技能补了几处模型侧能力**：改一台已配置的服务器（只改 URL 就只给 URL，其余字段与密钥原样保住）、只停某个工具、顺手写备注；技能能整目录启停、能改本插件自己写的那份。

- **人设多了「思考强度」**：在高级选项里与「模型」并排。档位是模型自己声明的，所以换模型要重新选；选了新模型之后旧档位若不在新清单里，会**自动清掉并告诉你**（留着下一次委派会被直接拒）。**指定了模型时留空不会沿用主会话那一档**，而是回落到该模型自己的默认。

- **提示词页的「生效中」修好了**：场景接管时，模型拿到的清单以前报"文件里是它"、界面报"场景绑的那份"，同一件事两个答案。现在两边同源，清单里还会标出"文件里是它、但基线由场景驱动"这种真实状态。

- **几处显示毛病**：记忆正文改成代码块包住（正文里手写的 `#` 以前会渲染成大标题、盖过场景分组）；子智能体高级选项折叠后的摘要固定排两行（不再把「高级选项」挤成竖排）；提示词预设的描述框补上字数上限。

- **给模型的话对齐了事实，指的路都能走通**：应用提示词的回执以前说"下个会话生效"，实际**下一轮就生效**（模型会白劝你重开会话）；场景保存的参数说明还引用着已被删掉的"注入后模型会照办"那句，改成了如实说明去向；工具描述与回执不再点名出厂默认关掉的工具（改用中性说法，或按开关在不在分叉——只在你真开了那条工具时才点名它）；两条清单工具的描述也对齐了各自真实给的东西。新增一条契约测试，把"描述点名被关工具"这类静默毛病挡在构建里。
---

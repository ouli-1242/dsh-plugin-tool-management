...
- **场景和记忆分家了**：以前它们挤在一个工具族里、也挤在一条注入里，现在各自独立 —— 工具上是两族（记忆 4 条 + 场景 1 条），注入上是两段（「场景」讲当前启用的是哪个、它是什么；「记忆」讲各场景下的条目）。好处有两个：**能单独关掉记忆那一半**（省字节）而保留场景说明；模型终于**能建场景、能给场景配 MCP / 技能 / 子智能体、能绑提示词**了（此前只能看着）。注意写场景档案**不会**让场景生效 —— 要不要用它，仍然由你在「场景」页决定。另外，场景那一段只在**启用了具体场景**时才会出现：当前就是「全局」默认状态时它不注入（说"当前处于全局"是废话，不该占上下文）。场景那一段现在是一行：`「代码」—— 写代码`（场景名 + 你写的描述），场景名不再重复三遍，也不再把它当成"要照办的约定" —— 它就是一句"这个场景是干什么的"。
...
- **注入里的套话削了一轮，场景那块也终于说了它在干嘛**：把四次实际注入摊开量了一遍 —— 四个板块 716 token/轮，**真正的内容只占 46%**，其余都是"我是谁、什么时候看我、以我为准"这类包装；越短的板块越离谱（场景 87%、MCP 76%，三行内容配了 98 token 的套话）。改了四处：记忆的授权语从"每个场景各说一遍"改成**整段只说一次**（还顺手把它从场景标题和条目中间挪走了 —— 它本来就该管整段）；附件那行去掉了"未注入正文、共 N 个"这类解释，只留路径和文件名（路径必须留，不然你让模型去读它读不到）；MCP 那块不再数"这台有几个工具"（模型拿这个数字什么也干不了）；"本份 X 取代更早注入的同类 X"这四句其实是同一条规则，现在统一成一句。
  另外**场景那块以前只说了它是什么，没说它干嘛** —— 它其实是"切进这个场景时，MCP / 技能 / 人设 / 记忆都按它筛过一遍"，现在这句话写进去了，四块之间的关系才连得上。这一处是**变贵**的（多了 10 token/轮），买的是语义清楚。四块合计 715 → 约 656 token/轮（省了 59）。

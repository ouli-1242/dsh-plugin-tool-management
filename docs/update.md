...
- **场景和记忆分家了**：以前它们挤在一个工具族里、也挤在一条注入里，现在各自独立 —— 工具上是两族（记忆 4 条 + 场景 1 条），注入上是两段（「场景」讲当前启用的是哪个、它是什么；「记忆」讲各场景下的条目）。好处有两个：**能单独关掉记忆那一半**（省字节）而保留场景说明；模型终于**能建场景、能给场景配 MCP / 技能 / 子智能体、能绑提示词**了（此前只能看着）。注意写场景档案**不会**让场景生效 —— 要不要用它，仍然由你在「场景」页决定。另外，场景那一段只在**启用了具体场景**时才会出现：当前就是「全局」默认状态时它不注入（说"当前处于全局"是废话，不该占上下文）。场景那一段现在只剩一行内容：`「代码」—— 用户描述：写代码`（场景名 + 你写的那句描述；没填描述就只显示场景名），场景名不再重复三遍，也不再把它当成"要照办的约定" —— 它就是一句"这个场景是干什么的"。中间那四个字是为了防止模型把破折号后面那截当成场景的另一个名字。

- **注入的排版分了层级**：每条注入现在是一级标题（板块：场景 / 记忆 / MCP / 技能 / 子智能体 / 提示词），记忆里的场景分组是二级、条目是列表 —— 一眼看得出主次。顺手修了个显示毛病：以前标题紧跟在标签那行后面、中间没有空行，渲染出来会跟上一行挤成一段（标题根本不成立）。`<system-reminder>` 那对标签保留不动 —— 那是官方约定的包法，去掉它模型会把这段读成你自己说的话。

- **注入里的套话全削了，只留"现在是什么"**：把四次实际注入摊开量了一遍 —— 四个板块 **823 token/轮**（真词表），**真正的内容只占 46%**，其余都是"我是谁、什么时候看我、以我为准"这类包装；越短的板块越离谱（场景 87%、MCP 76%，三行内容配了 98 token 的套话）。后来你给了一条原则，就照它把剩下的也砍了：**上下文注入就是当前的情况，目的是让模型知道现在的情况，不需要它知道没用的信息，反推更是浪费 token**。于是：
  - 六个板块开头那句加粗的"什么时候该想起我"（`先核对这里` / `先确认服务器在不在` / `先在这里找` / `先在这里选人设` …）**全删了** —— 它们是指令，不是当前情况；
  - 场景那块曾经补过一句"下面四段都已按它筛过"，也删了 —— 那是在解释"另外四段是怎么产生的"（机制），而且**只启用没进入场景时它是句假话**（场景"启用"和"进入"是两件事）；
  - MCP 那块不再说"工具名是 `mcp__<服务器>__<工具>`" —— 模型自己的工具表里就是这个命名，等于零信息量；
  - 记忆的授权语从"每个场景各说一遍"改成**整段只说一次**，还从场景标题和条目中间挪走了（它本来就该管整段）；
  - 附件那行去掉了"未注入正文、共 N 个"这类解释，只留路径和文件名（路径必须留，不然你让模型去读它读不到）；
  - MCP 那块不再数"这台有几个工具"（模型拿这个数字什么也干不了）；
  - "本份 X 取代更早注入的同类 X"这四句其实是同一条规则，统一成一句。

  现在四个板块的框架**只剩三样**：标题、一句补充说明（只有 MCP 和子智能体有）、"以本份为准"那句。四块合计 **823 → 643 token/轮（省了 180，−21.9%）**。顺带发现一件事：**一直用来数 token 的那个口径对中文不准** —— 它按"字符数 ÷ 4"算，对英文高估约 10%、对中文低估约 13%（工具表 18 条真值约 3,137，注入四块真值 823）。所以"工具表 3,439 / 注入 716"这两个数本来就不是同一把尺子；已在 `src/tools/table.ts` 的文件头写明，免得后来者拿两个数直接比。也顺手量过"改成英文会不会更省"：**不值得** —— 框架部分只省 8%（换个老词表能省 25%，所以结论取决于词表），而长句中文反而更省；更关键的是这段文本**你要在「注入实况」里审**，中文才审得动。

- **记忆条目前那句说明按你给的文案定稿**：现在是 `以下是当前场景对应的记忆；冲突时以实际情况和用户目前的对话为准；无关勿提。`（比最初那句少 60%）。三件事说清了：这是**当前场景对应的**记忆（所以下面才按场景分组）、和实际情况或你当场说的冲突时听后两者、无关的不必提。
  ⚠️ **代价如实说**：这一版删掉了「可能已过期」和结尾的"以下就是全部信息"。前一句是判断它与紧接的"冲突时以实际情况为准"重复 —— 成立；后一句原来是**防模型去翻工具找更多**的唯一手段，删掉之后，模型在清单里找不到某件事时可能会去调一次 `memory_manager_list`。**如果你发现这个行为变多**，把那半句加回常量末尾就行（`src/memories/constants.ts` 的 `SCENE_MEMORY_NOTE`）—— 结构没动，恢复是几行的事。

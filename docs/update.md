...
- **记忆条目前那句说明按你给的文案定稿**：现在是 `以下是当前场景对应的记忆；冲突时以实际情况和用户目前的对话为准；无关勿提；以下就是全部信息：`。三件事说清了：这是**当前场景对应的**记忆（所以下面按场景分组）、和实际情况或你当场说的冲突时听后两者、无关的不必提。结尾那句"以下就是全部信息"是**防模型去翻工具找更多**的手段 —— 它一度被删（当时记下了后果），同一天又按你的要求加回来，现在改成收尾式（分号 + 冒号），读起来是"下面就是全部内容"的引导语。
- **全局那一块不再叫「场景」**：记忆里的 `## 场景：全局` 改成 `## 常驻信息`。全局是恒常生效的保留桶、不是"一个场景" —— 写成场景会让模型以为它是当前场景的另一种取值。其余场景仍是 `## 场景：X`。

...
| 版本                         | 发布       | 主题                                                                                                                                                                      |
| ---------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [0.14.0](#0140---2026-09-23) | 2026-09-23 | 五族工具改名合并（20 → 17，旧开关自动搬家）+ 四个「保存」工具都是有则改无则建 + 人设思考强度 + 改 MCP 配置不再碰明文凭据 + 提示词「生效中」修好 + 描述框补上限 |
...
- **工具改名并合并：20 条变 17 条**。名字变了，能力没少、反而多了几项：

  - 记忆那一族改叫 `scene_memory_manager_*`（界面那一栏本来就叫「场景和记忆」，现在名字终于对上了），"新建"和"修改"合成一条 `_save`；
  - MCP 的"启停"和"重启"合成 `mcp_manager_switch`，用 `action` 选动词；
  - MCP、技能、子智能体三个"新建"都变成 `_save` —— **有则改、无则建**，回执会告诉你走了哪条；
  - 顺手多出来的能力：MCP 能只停某一个工具、能改一台已配置的服务器、能顺手写备注；技能能整目录启停、能改自己写的技能；提示词清单里能看到描述了。

- **旧名字不会当别名留着**：留一个别名就等于多一条工具常驻在每一轮请求里，八个旧名加起来比这次省下的还多 —— 那不是"多花一点"，是让这次改名白做。所以旧名直接失效。
...
- **体积**：整表从约 3,450 token 降到约 3,090，工具 20 → 17。注意 **MCP 那一族是变贵的**（627 → 704）—— 换来了"模型能改一台已配置服务器"，这是这次唯一一处花钱买能力的地方。

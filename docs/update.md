...
| 版本                         | 发布       | 主题                                                                                                                                                                      |
| ---------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [0.11.0](#0110---2026-09-19) | 2026-09-19 | 访问令牌可设置 / 关闭 / 修改 / 删除 + 没解锁时锁住输入框 + 批量启停可撤销 + 兼容页清理旧备份 + 两页可搜索 + 体检改为真调 + 一批凭据与数据损坏修复 + 搜索与列表提速 |
...
- **访问令牌能设置、能关掉、能改、也能彻底删掉了**：兼容页那块现在是一枚四态状态胶囊 + 一句实况，下面三行按"这一步管到哪儿"命名（本次启动 / 宿主配置 / 保护开关）。关掉只是"暂时不生效"，令牌留在配置里，随时开回来不用重填。

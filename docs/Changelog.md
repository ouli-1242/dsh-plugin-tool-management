...
- **契约测试 61 → 82 例**：新增客户端导出契约、装配与渲染（含带数据挂载与场景卡片文案契约）、技能状态文件读取韧性、技能删除权限、技能来源「移除来源」语义五组。
...
- **两版 README 的测试计数校正**：开发小节原写「十组 / 76 例」（英文版还写成 "ten groups / 76 cases"），实际 `npm test` 跑 **11 组 / 82 例**；英文版的组清单还漏列 `skills-delete`、`skills-state`、`skills-source-remove` 三组，中文版漏列 `skills-source-remove`。现两版均改为 11 组 / 82 例，并补上遗漏的组名与各自断言内容（含「移除来源」语义组与场景卡片文案契约）。

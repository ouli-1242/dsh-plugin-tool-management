...
**发布号说明**：npm 上此前最新是 **`0.5.0`**（2026-09-13 16:17:45 UTC 发布，`latest` 标签即它；再往前是 0.2.0 / 0.1.x）。
0.5.1 是紧随其后的**补丁版**：只含本节条目，0.5.0 已发布的功能说明见下一节与其后的「v0.5.0 详情」。

**给使用者的三句话**：场景页现在只放**专门设置的、可切换的预设场景**（「全局」不再出现——它恒定注入、
MCP/skills/人设各有专页）；场景档案弹窗的新加段**默认一项都不勾**（记忆段只预勾本场景里已启用的那几条）；
勾选行长描述不再横向溢出边框、筛选框不再贴死内容。

### 界面
- **档案弹窗：新加段默认不勾选 + 记忆描述截断 + 勾选行溢出修复**（用户反馈「MCP 工具集、技能集、子智能体绑定内都默认不勾选，记忆除了全局已经启动的勾选，其他也是不勾选；记忆描述要有截断，因为太长会导致文字超出边框；而且输入框和内容有重合」）：
  - **默认勾选**（推翻 v0.3 §2.4「预勾当前运行时状态」）：`MCP 工具集` / `技能集` 的「添加」由「照抄当前启用状态 / 勾上已启用技能」改为**空集**；`记忆` 的「添加」由「勾上全部」改为只勾**已启用**的那几条，`记忆` 的「全选」拆出来变成真正的全选（`allMemoryIds()`），原先它和「添加」是同一个子集。（作用域随后进一步收窄为「只认本场景」——见下条「记忆段只列被编辑场景自己的记忆」。）
  - **代价写进界面**：勾选集语义是「勾 = 启用」，空段 = 该域全部停用——所以现在点「添加 MCP 工具集」当场就等价于「该场景下 MCP 全停」。段脚注与弹窗顶部说明都写明这句话；**子智能体段例外**（空段 = 不限制，见 `filterBySceneBinding`），因此新增 `memory.archive.emptySubagents` 单独用词，不再复用「= 全部停用」的脚注。
  - **文字溢出段边框（真缺陷，已实测）**：`.dsm-pick-main` 只是个行内 `span`，名称与说明在同一行内联排布，而 `text-overflow:ellipsis` 对行内元素无效 → 长描述**横向溢出**。Playwright 量到：660px 宽的段里描述行宽 **976px、右侧超出段边框 353px**（档案弹窗变体超出 295px）。改为 `display:flex;flex-direction:column`（名称/说明各占一行、各自单行省略）后同一测量为 **-55px / -40px**（不再溢出），行高 54 → 49px。
  - **记忆描述截断**：勾选行里的记忆描述按 **80 字**（`MEM_DESC_MAX`）截断，全文进 `title` 悬浮提示；`pickRow` 新增 `descTitle` 支持。
  - **输入框与内容贴死**：筛选框原来紧贴第一条（间距实测 **0px**）→ `.dsm-seg-filter` 底部补 8px（实测 8px）；另加 `.dsm-seg-body>*{flex:none}` 与 `.dsm-tools-grid>*{flex:none}`，禁止滚动容器把子项压缩（压缩是这类「叠在一起」的常见成因）。
- **场景页（场景档案）排版重做**（用户反馈「全局框只显示描述，不显示当前记忆、skill 等这些数量，描述也要有字数限制，不然会导致全局变成纵向布局；场景档案的排版、交互、组件都很杂乱，丑陋」）：
  - **卡片不再挂数量**。原卡片把「描述 + N 条记忆 + 已配：MCP N 台 · 技能 N 个 · 子智能体 N 个 · 记忆 N 条」全塞在一行，且 `dsm-path` 右贴——一行里三类信息互相挤，长描述直接把卡片撑成纵向。现在卡片只有四样：名字（+ 磁盘目录名 + 状态标签 + 开关）、**一行描述**、动作行（主按钮 + 右侧次按钮）。数量统一收进页首新增的「**当前模式**」条（`dsm-mode-bar`：圆点 + 当前模式名 + 档案摘要 + 退出按钮）。
  - **「当前模式」条只在进入模式后出现**（用户反馈「自由模式 / 各场景按自己的启用开关注入记忆…这个是干什么的，感觉没什么用」）：原先没有模式时常驻一条「自由模式 + 一句解释」，纯静态解释占一整条，而「没有条 = 没进任何模式」本就不言自明。现在它只承载有状态的信息（哪个场景是当前模式 / 它包含什么档案 / 退出按钮），`scenes.mode.free` 与 `scenes.mode.free.hint` 两个词条一并删除。
  - **描述字数上限 60**：新建/编辑表单加 `maxLength` 与 `n/60` 计数，显示层再 `clipText` 裁一次兜住历史数据（卡片 `text-overflow:ellipsis` 单行，全文进 `title`）。
  - **档案弹窗「记忆」段的场景卡片**：`.dsm-pick-main` 是行内 `span`，名称与说明会**同行内联换行**（这正是「纵向布局」的成因）→ 显式改为纵向 flex，两行各自单行省略。保留场景「全局」恒定注入，「已勾 N/M」对它没有意义，**它的卡片只显示描述**（`sceneMemDesc`）；其余场景仍显示「描述 · 勾选进度」。
    > **本节后续已废弃**：这一段整体改成「只列被编辑场景自己的记忆」的平铺勾选行，场景卡片、`sceneMemDesc` 与「选记忆」钻取都已删除，见下一条。
  - **记忆段只列被编辑场景自己的记忆**（用户裁定：「其他场景的记忆也不需要显示全局」）：原先这一段列**所有场景的卡片** + 「选记忆」钻取，但宿主侧 `memoryAllowed(archives, file.scene, id)` 是按**记忆所属场景**取档案（`src/rules/archive.ts:135`，渲染调用点 `src/rules/service.ts:1044`；回归测试 `hub-layout.test.mjs`「保留场景 global 的记忆不受其它场景的勾选段影响」）——**A 场景档案里的 memories 段只能门控 A 自己的记忆**，勾别的场景（含全局）完全不生效，列出来就是陷阱。现改为与本弹窗其余三段同构的平铺勾选行：`sceneMemoriesOf(memories, scene)` 只取本场景、并按一级目录归场景（顺带修掉「多级分组的记忆整个看不见」——原先按完整 group 比对，`办公/流程/站会` 归不到 `办公`）；「全选」= 本场景全部；「添加」的默认勾选由「全局已启用」改为**本场景已启用**（`memDefaultPickIds(memories, rules, scene)`）；场景卡片、钻取视图、`memDrill` 状态、7 条相关词条与 `.dsm-scene-card*` 五条 CSS 一并删除。
  - **交互**：卡片改成响应式网格（`repeat(auto-fill,minmax(300px,1fr))`，容器窄于 640px 单列）；「切入此模式」从次要按钮升为该卡的主按钮（有档案才出现，且当前模式那张不显示）；历史保留场景 `_shared`（「常开」）不出开关与删除。
  - **场景页不再列保留场景「全局」**（用户裁定：「全局记忆都不用设置场景就能注入上下文，如果我需要设置 skills、mcp，我直接去具体页设置就好了，所以说场景不需要展示全局，不需要设置全局，只有有特定需求才需要设置专门的场景。」）：`presetScenes = data.scenes.filter(s => s.global !== true)` —— 页面只放**可切换的预设场景**，三个统计（`个场景` / `个已启用` / `个有档案`）同步只数可见场景；只剩 `global` 时显示空态（词条 `scenes.empty` 改写为「还没有专门设置的场景；全局记忆不需要场景就能注入，需要切换 MCP / 技能 / 人设时再新建。」）。**过滤只做在场景页这一层**：宿主回传的 `scenes` 仍含全局，记忆页的全局分组与各种场景选择器照旧（否则全局记忆在界面上就没有落点）。代价（用户接受）：全局的**场景描述与档案**不再有 UI 入口——MCP/skills 各有专页，不需要"全局档案"这层间接。
  - **标签文案**：保留场景的标记从「任何对话都注入」缩成「**常驻**」（六个字压成两个字）；档案弹窗「记忆」段里同一含义的「始终注入」一并统一为「常驻」，同一件事全站只有一种说法。英文侧对应 `always injected`（`_shared` 的「常开 / always on」保持不变，两者语义不同）。「常驻」现在只出现在**记忆页**的全局分组（场景页不列全局、档案弹窗记忆段也不再出现全局）。
  - 验收证据（静态预览渲染，**非浏览器活体验收**）：以假 ctx + 真实 `dict` 渲染 `ScenesPage` 到静态 HTML（连同 bundle 里的真实 CSS），900px 两列 / 600px 单列各出一张截图逐项核对；另在预览里手工拼出档案弹窗「记忆」段（含超长描述）确认卡片保持横向、描述单行省略。契约测试守住：描述超长必裁（含 CSS 断言 `white-space:nowrap` / `flex-direction:column` 与 `maxLength`）、**全局不出现在场景页**（两种模式状态各渲染一遍断言全文无「全局」「常驻」，预设场景仍在）、**只剩全局时为空态且零卡片**、**记忆段只认本场景的记忆**（全局与其它场景都取不到、多级分组按一级目录归位、默认勾选只含本场景已启用、源码守卫禁止场景卡片与钻取复活）。

### 文档与测试
- **契约测试 81 → 85 例**：新增/扩写「档案弹窗默认勾选」「记忆段只认本场景的记忆」「场景页不列全局 + 只剩全局时空态」「模式条状态」等客户端契约。**0.5.0 的文档写 76 例是滞后值**（发布时实际已 81 例），本次一并校正。
- **两版 README 的测试计数校正**：开发小节原写「十组 / 76 例」（英文版还写成 "ten groups / 76 cases"），英文版的组清单还漏列 `skills-delete`、`skills-state`、`skills-source-remove` 三组，中文版漏列 `skills-source-remove`。现两版均改为 11 组 / 85 例，并补上遗漏的组名与各自断言内容。
...
**已发布**：npm `dsh-plugin-tool-management@0.5.0`（2026-09-13 16:17:45 UTC）。本节与下一节「v0.5.0 详情」即该版本的内容。
...
- **技能来源新增「移除来源」**（本版新增，详细条目见下一节 §界面）：把某个来源从**读取范围**里摘掉——不再扫描该目录、技能页不列出、模型侧候选也不产出；**源目录与文件一个字节都不动**，界面单列一组「已移除的来源」可一键「恢复读取」。`DSH 技能` 与 `导入技能` 是保留来源，**不可移除**（否则新建/导入没有落点）。
- **界面**：工具页七标签（场景 / MCP / 技能 / 子智能体 / 提示词 / 记忆 / 会话）；场景卡片补描述与绑定摘要；场景档案编辑器新增第 4 段「记忆」；人设表单的模型与工具限制收进「高级选项」（模型下拉 + 白/黑名单勾选器）。
- **契约测试 61 → 76 例**：新增客户端导出契约、装配与渲染（含带数据挂载）、技能状态文件读取韧性、技能删除权限四组。（文档口径滞后：发布时实际已 **81 例**，0.5.1 一并校正，见上节。）
...
- **场景页**：卡片补齐「描述 + 记忆条数 + 已绑定摘要（MCP/技能/子智能体/记忆各多少）+ 磁盘目录名」；新增「改描述」（`rules-update-scene`）；保留场景不显示删除/停用开关；统计标签从硬编码中文改走 i18n。
  > **0.5.1 已取代**：用户反馈这些数量让卡片「很杂乱、丑陋」，且长描述会把卡片撑成纵向 → 卡片改为「名字/标签/开关 + 一行描述 + 动作行」，数量移入页首「当前模式」条，描述限 60 字。见上节「场景页（场景档案）排版重做」。
...
- **场景档案编辑器**：新增第 4 段「记忆」（`archive.memories`，id = `<场景>/<名>`）。勾选语义与其余段同构——**段未定义 = 不碰；段已定义但没勾的记忆不进提示词**；勾选**只影响投影**，记忆文件与内容一律不动。场景改成卡片（描述 + 已勾/总数 + 「始终注入」标记），点「选记忆」进场景内的记忆明细钻取视图。四段都加筛选框。
  > **0.5.1 已收窄**：保留场景「全局」恒定注入，「已勾 N/M」对它没有意义 → 该卡片只显示描述；名称与说明在 `.dsm-pick-main` 里由行内换行改为各占一行（长描述不再把卡片撑成纵向）。
  > **0.5.1 已改**：「添加」不再预勾当前启用状态（改为空集 / 记忆只预勾本场景里已启用的那几条），记忆描述按 80 字截断；场景页也不再列保留场景「全局」，档案弹窗记忆段只列本场景的记忆。理由与实测数据见上节「档案弹窗：新加段默认不勾选…」「场景页（场景档案）排版重做」「记忆段只列被编辑场景自己的记忆」。
...
- **技能来源改名与删除权限**（用户裁定）：
  ...
  - **「移除来源」= 不再读取它**（用户澄清：「是不读取这个文件夹了，不是把文件夹删除」/「不是删除，是移除可以读取」）。原来的「停用来源」仍会读取并列出技能（只是不可调用），而移除是**连目录都不扫**：
    - 新增状态字段 `removedSources`（可持久、可恢复）。`state()` 把这些来源的技能全部排除，`listProviderCandidates()` 也不再产出它们的候选 —— 所以**模型侧同样看不到**；
    - 源目录与文件一个字节都不动；已移除的来源仍在 `roots` 里带 `removed: true` 返回，界面单列一组「已移除的来源」，可一键「恢复读取」；
    - `dsh`（官方技能目录）与 `hub`（导入落点）**不可移除**（`error.source.reserved`）—— 移除会让创建/导入无处落脚；
    - 新 op：`skill-source-remove` / `skill-source-restore`；来源行新增 `removable` 字段，技能页的「移除来源」按钮据此显示。回归测试 `test/skills-source-remove.test.mjs`（4 例）断言「移除后技能不再被读取、源文件原封不动、恢复后又能读到」，并单独断言**移除 ≠ 停用**。

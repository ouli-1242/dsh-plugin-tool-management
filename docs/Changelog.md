...
- **契约测试 61 → 84 例**：新增客户端导出契约、装配与渲染（含带数据挂载、场景卡片文案契约、档案弹窗默认勾选契约、模式条状态契约）、技能状态文件读取韧性、技能删除权限、技能来源「移除来源」语义五组。
...
- **两版 README 的测试计数校正**：开发小节原写「十组 / 76 例」（英文版还写成 "ten groups / 76 cases"），实际 `npm test` 跑 **11 组 / 84 例**；英文版的组清单还漏列 `skills-delete`、`skills-state`、`skills-source-remove` 三组，中文版漏列 `skills-source-remove`。现两版均改为 11 组 / 84 例，并补上遗漏的组名与各自断言内容（含「移除来源」语义组、场景卡片文案契约、档案弹窗默认勾选契约、模式条状态契约）。
...
- **场景页（场景档案）排版重做**（用户反馈「全局框只显示描述，不显示当前记忆、skill 等这些数量，描述也要有字数限制，不然会导致全局变成纵向布局；场景档案的排版、交互、组件都很杂乱，丑陋」）：
  ...
  - **场景页不再列保留场景「全局」**（用户裁定：「全局记忆都不用设置场景就能注入上下文，如果我需要设置 skills、mcp，我直接去具体页设置就好了，所以说场景不需要展示全局，不需要设置全局，只有有特定需求才需要设置专门的场景。」）：`presetScenes = data.scenes.filter(s => s.global !== true)` —— 页面只放**可切换的预设场景**，三个统计（`个场景` / `个已启用` / `个有档案`）同步只数可见场景；只剩 `global` 时显示空态（词条 `scenes.empty` 改写为「还没有专门设置的场景；全局记忆不需要场景就能注入，需要切换 MCP / 技能 / 人设时再新建。」）。**过滤只做在场景页这一层**：宿主回传的 `scenes` 仍含全局，记忆页的全局分组、新建/导入记忆的场景选择器、档案弹窗「记忆」段全部照旧（否则全局记忆在界面上就没有落点）。代价（用户接受）：全局的**场景描述与档案**不再有 UI 入口——MCP/skills 各有专页，不需要"全局档案"这层间接。
  - **标签文案**：保留场景的标记从「任何对话都注入」缩成「**常驻**」（六个字压成两个字）；档案弹窗「记忆」段里同一含义的「始终注入」一并统一为「常驻」，同一件事全站只有一种说法。英文侧对应 `always injected`（`_shared` 的「常开 / always on」保持不变，两者语义不同）。「常驻」现在只出现在**记忆页**的全局分组与档案弹窗的记忆段（场景页已不列全局）。
  - 验收证据（静态预览渲染，**非浏览器活体验收**）：以假 ctx + 真实 `dict` 渲染 `ScenesPage` 到静态 HTML（连同 bundle 里的真实 CSS），900px 两列 / 600px 单列各出一张截图逐项核对；另在预览里手工拼出档案弹窗「记忆」段（含超长描述）确认卡片保持横向、描述单行省略。契约测试守住：描述超长必裁（含 CSS 断言 `white-space:nowrap` / `flex-direction:column` 与 `maxLength`）、**全局不出现在场景页**（两种模式状态各渲染一遍断言全文无「全局」「常驻」，预设场景仍在）、**只剩全局时为空态且零卡片**。

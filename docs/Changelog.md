...
## v0.5.0 — 统一数据目录 + 场景显式化 + 界面重做（2026-09-13）

> **版本号对应关系**（避免对上号时困惑）：npm 上已发布的是 `0.1.1`；本仓库源码此前写作 `0.2.0`，
> 而本文档当时用 `v0.4` 编号——三套号各说各话。自本次发布起统一为 **`0.5.0`**：
> `package.json` = npm 发布号 = 本节标题。下面三节（`v0.4` / `v0.3` / `v0.2 及更早`）是当时的历史
> 编号，保留不改写；**文中出现的 "v0.4 起…" 这类表述，指的就是 0.5.0 这次发布引入的行为**。

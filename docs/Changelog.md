...
- **契约测试 61 → 83 例**：新增客户端导出契约、装配与渲染（含带数据挂载、场景卡片文案契约、档案弹窗默认勾选契约）、技能状态文件读取韧性、技能删除权限、技能来源「移除来源」语义五组。
...
- **两版 README 的测试计数校正**：开发小节原写「十组 / 76 例」（英文版还写成 "ten groups / 76 cases"），实际 `npm test` 跑 **11 组 / 83 例**；英文版的组清单还漏列 `skills-delete`、`skills-state`、`skills-source-remove` 三组，中文版漏列 `skills-source-remove`。现两版均改为 11 组 / 83 例，并补上遗漏的组名与各自断言内容（含「移除来源」语义组、场景卡片文案契约、档案弹窗默认勾选契约）。
...
- **场景页（场景档案）排版重做**（用户反馈「全局框只显示描述，不显示当前记忆、skill 等这些数量，描述也要有字数限制，不然会导致全局变成纵向布局；场景档案的排版、交互、组件都很杂乱，丑陋」）：
  - **卡片不再挂数量**。原卡片把「描述 + N 条记忆 + 已配：MCP N 台 · 技能 N 个 · 子智能体 N 个 · 记忆 N 条」全塞在一行，且 `dsm-path` 右贴——一行里三类信息互相挤，长描述直接把卡片撑成纵向。现在卡片只有四样：名字（+ 磁盘目录名 + 状态标签 + 开关）、**一行描述**、动作行（主按钮 + 右侧次按钮）。数量统一收进页首新增的「**当前模式**」条（`dsm-mode-bar`：圆点 + 当前模式名 + 档案摘要 + 退出按钮；未进入模式时显示「自由模式」与一句说明）。

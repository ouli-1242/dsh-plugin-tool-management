...
**发布号说明**：npm 上此前最新是 `0.1.1`；**`0.5.0` 从未发布**（只存在于版本库），0.5.x 的**首个发布版是 `0.5.1`**，
本节即该版本的全部内容，标题（含下一节详情）由 v0.5.0 改为 v0.5.1，条目不变。

本次发布把此前散落的四套变更合并成一个版本，并修掉一轮真实故障。**详细条目见下一节「v0.5.1 详情」**
...
- **契约测试 61 → 85 例**：新增客户端导出契约、装配与渲染（含带数据挂载、场景卡片文案契约、档案弹窗默认勾选契约、模式条状态契约）、技能状态文件读取韧性、技能删除权限、技能来源「移除来源」语义五组。
...
- **两版 README 的测试计数校正**：开发小节原写「十组 / 76 例」（英文版还写成 "ten groups / 76 cases"），实际 `npm test` 跑 **11 组 / 85 例**；英文版的组清单还漏列 `skills-delete`、`skills-state`、`skills-source-remove` 三组，中文版漏列 `skills-source-remove`。现两版均改为 11 组 / 85 例，并补上遗漏的组名与各自断言内容（含「移除来源」语义组、场景卡片文案契约、档案弹窗默认勾选契约、模式条状态契约）。
- **场景档案弹窗：新加段默认不勾选 + 记忆描述截断 + 勾选行溢出修复**（用户反馈「MCP 工具集、技能集、子智能体绑定内都默认不勾选，记忆除了全局已经启动的勾选，其他也是不勾选；记忆描述要有截断，因为太长会导致文字超出边框；而且输入框和内容有重合」）：
  - **默认勾选**（推翻 v0.3 §2.4「预勾当前运行时状态」）：`MCP 工具集` / `技能集` 的「添加」由「照抄当前启用状态 / 勾上已启用技能」改为**空集**；`记忆` 只预勾**保留场景「全局」里已启用**的那几条（`memDefaultPickIds(memories, rules)`，启用状态取自客户端本来就请求的 `rules-list`，**不需要宿主配合**）。`记忆` 的「全选」拆出来变成真正的全选（`allMemoryIds()`），原先它和「添加」是同一个子集。
...
- **场景页（场景档案）排版重做**（用户反馈「全局框只显示描述，不显示当前记忆、skill 等这些数量，描述也要有字数限制，不然会导致全局变成纵向布局；场景档案的排版、交互、组件都很杂乱，丑陋」）：
  ...
  - **档案弹窗「记忆」段的场景卡片**：`.dsm-pick-main` 是行内 `span`，名称与说明会**同行内联换行**（这正是「纵向布局」的成因）→ 显式改为纵向 flex，两行各自单行省略。保留场景「全局」恒定注入，「已勾 N/M」对它没有意义，**它的卡片只显示描述**（`sceneMemDesc`）；其余场景仍显示「描述 · 勾选进度」。
    > **同版后续已废弃**：这一段整体改成「只列被编辑场景自己的记忆」的平铺勾选行，场景卡片、`sceneMemDesc` 与「选记忆」钻取都已删除，见下一条。
...
## v0.5.1 详情 — 统一数据目录 + 场景显式化 + 界面重做（2026-09-13）
...
- **场景页**：卡片补齐「描述 + 记忆条数 + 已绑定摘要（MCP/技能/子智能体/记忆各多少）+ 磁盘目录名」；新增「改描述」（`rules-update-scene`）；保留场景不显示删除/停用开关；统计标签从硬编码中文改走 i18n。
  > **同版后续已被取代**：用户反馈这些数量让卡片「很杂乱、丑陋」，且长描述会把卡片撑成纵向 → 卡片改为「名字/标签/开关 + 一行描述 + 动作行」，数量移入页首「当前模式」条，描述限 60 字。见上节「场景页（场景档案）排版重做」。
...
- **场景档案编辑器**：新增第 4 段「记忆」（`archive.memories`，id = `<场景>/<名>`）。勾选语义与其余段同构——**段未定义 = 不碰；段已定义但没勾的记忆不进提示词**；勾选**只影响投影**，记忆文件与内容一律不动。场景改成卡片（描述 + 已勾/总数 + 「始终注入」标记），点「选记忆」进场景内的记忆明细钻取视图。四段都加筛选框。
  > **同版后续已收窄**：保留场景「全局」恒定注入，「已勾 N/M」对它没有意义 → 该卡片只显示描述；名称与说明在 `.dsm-pick-main` 里由行内换行改为各占一行（长描述不再把卡片撑成纵向）。
  > **同版后续已改**：「添加」不再预勾当前启用状态（改为空集 / 记忆只预勾「全局」已启用的那几条），记忆描述按 80 字截断；场景页也不再列保留场景「全局」。理由与实测数据见上节「场景档案弹窗：新加段默认不勾选…」与「场景页（场景档案）排版重做」。

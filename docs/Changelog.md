...
- **场景页（场景档案）排版重做**（用户反馈「全局框只显示描述，不显示当前记忆、skill 等这些数量，描述也要有字数限制，不然会导致全局变成纵向布局；场景档案的排版、交互、组件都很杂乱，丑陋」）：
  ...
  - **交互**：卡片改成响应式网格（`repeat(auto-fill,minmax(300px,1fr))`，容器窄于 640px 单列）；「切入此模式」从次要按钮升为该卡的主按钮（有档案才出现，且当前模式那张不显示）；保留场景（`global` / `_shared`）不出开关与删除。
  - **标签文案**：保留场景的标记从「任何对话都注入」缩成「**常驻**」（六个字压成两个字，卡片头部不再被标签占满）；档案弹窗「记忆」段里同一含义的「始终注入」一并统一为「常驻」，同一件事全站只有一种说法。英文侧对应 `always injected`（`_shared` 的「常开 / always on」保持不变，两者语义不同）。
  - 验收证据（静态预览渲染，**非浏览器活体验收**）：以假 ctx + 真实 `dict` 渲染 `ScenesPage` 到静态 HTML（连同 bundle 里的真实 CSS），900px 两列 / 600px 单列各出一张截图逐项核对；另在预览里手工拼出档案弹窗「记忆」段（含超长描述）确认卡片保持横向、描述单行省略。契约测试 `场景卡片文案：全局只显示描述（无任何数量）…描述超长必裁` 守住这三条（含 CSS 断言 `white-space:nowrap` / `flex-direction:column` 与 `maxLength`）。
...
- **场景档案编辑器**：新增第 4 段「记忆」（`archive.memories`，id = `<场景>/<名>`）。勾选语义与其余段同构——**段未定义 = 不碰；段已定义但没勾的记忆不进提示词**；勾选**只影响投影**，记忆文件与内容一律不动。场景改成卡片（描述 + 已勾/总数 + 「始终注入」标记），点「选记忆」进场景内的记忆明细钻取视图。四段都加筛选框。
  ...
  > **同版后续已改**：「添加」不再预勾当前启用状态（改为空集 / 记忆只预勾「全局」已启用的那几条），记忆描述按 80 字截断。理由与实测数据见上节「场景档案弹窗：新加段默认不勾选…」。
